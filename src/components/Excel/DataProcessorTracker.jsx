...
const DataProcessorTracker = ({ programId, isLoading }) => {
  const { data: currentProgram } = useDataQuery(currentProgramQuery, {
    variables: { programId },
  });
  ...
  useEffect(() => {
    if (
      ...
      //* Program Settings
      programMetadata = JSON.parse(
        currentProgram.results.attributeValues.find(
      ...
      programTET = currentProgram.results.withoutRegistration
        ? 'Not Applicable for Event Programs'
        : currentProgram.results.trackedEntityType?.name || 'N/A';
      programCatCombo = currentProgram.results.categoryCombo.name;
      programType =
        (currentProgram.results.withoutRegistration ? 'Event' : 'Tracker') +
        ' Program';

      //* Options Map
      optionData =
        optionSets.results?.optionSets?.map((os) => {
      ...
      trackedEntityAttributesData =
        trackedEntityAttributes.results.trackedEntityAttributes?.map((tea) => {
          return {
            ref: tea.name,
            name: tea.name,
            id: tea.id,
      ...
      deProperties.results.properties?.forEach((property) => {
        if (property.name == 'aggregationType') {
          aggTypes = property.constants.map((at) => ({
            name: DHIS2_AGG_OPERATORS_MAP[at] || at,
            value: at,
          }));
        }
        ...
        if (property.name == 'valueType') {
          valueTypes = property.constants.map((vt) => ({
            name: DHIS2_VALUE_TYPES_MAP[vt] || vt,
            value: vt,
          }));
        }
      });
  ...
  const setStagesContents = () => {
    currentProgram.results?.programStages
      ?.sort((stageA, stageB) => {
        if (stageA.sortOrder > stageB.sortOrder) {
          return 1;
        }
        if (stageA.sortOrder < stageB.sortOrder) {
          return -1;
          ...
          programSection.dataElements.forEach((dataElement) => {
            ...
            const metadata =
              metadataString.length > 0
                ? JSON.parse(metadataString[0].value)
                : {};
            ...
            const psdeCompulsory = programStage.programStageDataElements?.find(
  ...
  const setTEAContents = () => {
    ...
    unusedTEAs = allTEAs.filter(
      (tea) => !teaConfigurations.find((elem) => elem.id === tea.id)
    );
      ...
      const additionalConfigs = [
        {
          ...
          form_name:
            programSections.length == 0
              ? 'Basic Form'
              : 'Available TEAs not in Form',
          program_section_id:
            programSections.length == 0 ? 'basic-form' : 'unused',
        },
      ];
