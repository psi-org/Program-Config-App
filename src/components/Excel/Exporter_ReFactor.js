...
const Exporter_ReFactor = (props) => {
    ...
    const generate = () => {
        ...
        addConfigurations(hnqisType_Mwi, templateWS); // 'Template' sheet populate
...
export default Exporter_Refactor;


// -----------------------------------------
// -- Internal Methods ---------------------

const columnDef_Template = [
    ...
    { header: "Critical Step", key: "isCritical", width: 16},
    { header: "Compulsory", key: "isCompulsory", width: 16},
    ...
    { header: "Feedback Order", key: "compositive_indicator", width: 22, style: { numFmt: "@" } },
    ...
    { header: "Errors/Warnings/Info", key: "prompts", width: 50}
];

// TODO: We can make other elements as definition array as well.

// CHANGE # =============================
const isHnqisTypeMwi = ( hnqisType ) => { return ( hnqisType === "HNQISMWI" ); };

const getHiddenColumns = ( hnqisType_Mwi ) => {
    const hideColumnNames = ['program_stage_id', 'program_section_id', 'data_element_id'];

    // Extract keys with value as "X"
    const templateHeader = hnqisType_Mwi ? HNQISMWI_HEADER_INSTRUCTIONS : HNQIS2_HEADER_INSTRUCTIONS;
        
    const keysWithX = Object.keys(templateHeader).filter(  key => templateHeader[key] === "X" );
...
// 'Template' sheet populate
const addConfigurations = ( hnqisType_Mwi, ws ) => {
    ws.columns = columnDef_Template;
    ...
    // CHANGE #1 hnqisType_Mwi ==================================
    ws.getRow(2).values = hnqisType_Mwi ? HNQISMWI_HEADER_INSTRUCTIONS: HNQIS2_HEADER_INSTRUCTIONS;
    ws.getRow(2).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "D9D9D9" } };
    ...
    addValidation(hnqisType_Mwi, ws);

    addConditionalFormatting(hnqisType_Mwi, ws);
...
// CHANGE #1 hnqisType_Mwi ==================================
const addValidation = (hnqisType_Mwi, ws) => {
    ...
    dataValidation(ws, "B3:B3000", {
        ...
        formulae: hnqisType_Mwi ? structureValidatorMWI : structureValidator
    });
    ...
    dataValidation(ws, "G3:G3000", {
        type: 'list',
        // allowBlank: true,
        allowBlank: ( hnqisType_Mwi ) ? false: true,
        error: 'Please select the valid value from the dropdown',
        errorTitle: 'Invalid Selection',
        showErrorMessage: true,
...
const addConditionalFormatting = (hnqisType_Mwi, ws) => {
    const validationsList = HNQIS2_CONDITIONAL_FORMAT_VALIDATIONS;

    // CHANGE #3 =====================================
    const hiddenColumns = getHiddenColumns( hnqisType_Mwi );
