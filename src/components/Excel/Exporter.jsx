...
const Exporter = (props) => {
  ...
  const addInstructions = async (ws) => {
    ...
    ws.getCell('B18').value =
      "These settings will not affect this Template's behavior; however, they will update the Program configurations in the DHIS2 Server.";
    ...
    const healthAreaFound = props.healthAreaData.find(
      (ha) => ha['code'] == props.programHealthArea
    );
    ws.getCell('C16').value = healthAreaFound
      ? healthAreaFound['Health Area']
      : 'Family Planning';
    ...
    editingCell.cell.value =
      'Program Name: The Name of the Program (Checklist).';
    editingCell = buildCellObject(ws, 'F13:H13');
    editingCell.merge();
    editingCell.cell.value =
      'Program Short Name: The Short Name property of the Program (Checklist).';
    editingCell = buildCellObject(ws, 'F14:H14');
    editingCell.merge();
    editingCell.cell.value =
      "Use 'Competency Class': This will determine if competency classes will be included in the Program.";
    editingCell = buildCellObject(ws, 'F15:H15');
    editingCell.merge();
    editingCell.cell.value =
      'DE Prefix: Prefix that will be added to every Data Element contained in the Checklist. Must be unique.';
    editingCell = buildCellObject(ws, 'F16:H16');
    editingCell.merge();
    editingCell.cell.value =
      'Health Area: The Health Area assigned to the Checklist, used for filtering.';
    ...
    ws.getCell('B24').value =
      'Most of the configurations are performed in the "Template" tab, which contains several columns described below:';
    ...
    ws.getCell(
      'B42'
    ).value = `Every row in the 'Template' Tab starting from row 3 will generate the necessary components for the program when this file gets imported to the server. `;
    ws.getCell(
      'B43'
    ).value = `The template will highlight in red some cells automatically while you are working on it, this means that there is a validation error in that cell and you must fix it before importing.`;
    ws.getCell(
      'B44'
    ).value = `Also, keep in mind that this file is protected so you can only modify some specific cell ranges (most of the unlocked cells are in the 'Template' Tab).`;
    ...
    editingCell.cell.value =
      "'=Instructions!C12' cell will be highlighted in red.";
    editingCell.cell.alignment = verticalMiddle;

    editingCell = buildCellObject(ws, 'E50');
    editingCell.cell.value =
      'A Dialog Box will show up with the proper message.';
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      "Only 'Yes' or 'No' can be selected. A Dialog Box will be displayed when invalid data is provided.";
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      'If the provided value does not match any of the available options, a Dialog Box will be displayed';
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      "Either 'Section', 'question', 'label', or 'score' can be selected. A Dialogbox will show to select the proper valid option from the list.";
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      'Only the available options are accepted as input.';
    editingCell.cell.alignment = verticalMiddle;

    editingCell = buildCellObject(ws, 'C57');
    editingCell.cell.value =
      "'Score Numerator' and 'Score Denominator' columns";
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      'The cell will be highlighted in red if the corresponding Structure is present but the Form Name is empty.';
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      "If the 'Structure' is 'score', a Compositive Indicator must be provided.";
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      "If either 'Numerator' or 'Denominator' is present, the 'Feedback Order' should be provided; otherwise, the corresponding cell will be highlighted in red.";
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      'If any of the scores is present and the other one is missing, the missing score will be highlighted in red.';
    editingCell.cell.alignment = verticalMiddle;
    ...
    editingCell.cell.value =
      'If the Parent Question is defined, the Answer Value must be defined as well (and vice versa). The missing value will be highlighted in red.';
    editingCell.cell.alignment = verticalMiddle;

    ws.getCell(
      'B64'
    ).value = `Please ensure the Template content is clean. Everything will appear in the Program just as it is submitted to the server. The developers will not correct any spelling mistakes or spacing errors. `;

    ws.getCell(
      'B66'
    ).value = `Once you are done with the configurations, proceed by uploading this template to the Program Configuration App to the same server where it was downloaded from to process it.`;
    ws.getCell(
      'B67'
    ).value = `After uploading, please check that the content is correct by using the PCA Preview feature, then proceed to save the changes.`;
    ...
    ws.getCell(
      'B70'
    ).value = `Test your configurations on both the Web Tracker Capture and the Android Capture App.`;
    ws.getCell(
      'B71'
    ).value = `If necessary, download the Template again to make further changes.`;
    ws.getCell(
      'B72'
    ).value = `Remember that once uploaded, you can download a copy of this template from the server.`;
  ...
  const addConfigurations = (ws) => {
    ...
    ws.getRow(2).values = {
      ...
      isCritical:
        "A critical step will count for the Critical Score\n[Default is 'No']",
      isCompulsory:
        "A ompulsory Question must be answered to complete an assessment\n[Default is 'No']",
      ...
      compositive_indicator:
        'This number will generate the feedback tree in the app, accepted values are:1, 1.1, 1.1.1, 1.1.2, 1.1..., 1.2, etc.',
      parent_question:
        'Select the Parent Name of the Question that will act as parent',
    ...
    ws.getCell('A2').note = {
      texts: [
        {
          text: 'If the Parent Name is not automatically generated for questions, then drag the formula from another cell in the same column.',
        },
      ],
      margins: {
        insetmode: 'custom',
  ...
  const addValidation = (ws) => {
    ...
    dataValidation(ws, 'I3:J3000', {
      ...
      formulae: [0],
      error:
        'The numerator or denominator for the specified question have to be greater that zero',
  ...
  const addMapping = async (ws) => {
    ...
    defineName(
      ws,
      `L2:M${props.healthAreaData.length + 2}`,
      'Health_Area_Data'
    );
    defineName(
      ws,
      `M3:M${props.healthAreaData.length + 2}`,
      'Health_Area_Option'
    );
    defineName(ws, `O2:P${props.legendSetData.length + 2}`, 'Legend_Set_Data');
    defineName(
      ws,
      `O3:O${props.legendSetData.length + 2}`,
      'Legend_Set_Option'
    );
