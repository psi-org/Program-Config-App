...
const OunitScreen = ({ id, readOnly, setOrgUnitProgram, setNotification }) => {
  ...
  useEffect(() => {
    if (!poLoading && userOrgUnits) {
      const ouPaths = [
        ...prgOrgUnitData.results?.organisationUnits.map((ou) => ou.path),
      ];
      const ouIds = [
  ...
  const selectOrgUnits = (data) => {
    ...
    setSelectedOrgUnits((prevState) =>
      concatArraysUnique(prevState, ounitsIdList)
    );
    setOrgUnitPathSelected((prevState) =>
      concatArraysUnique(prevState, ounitsPathList)
    );
  ...
  const orgUnitAssignmentHandler = () => {
    ...
    if (!metadataLoading) {
      ...
      metadata.programs[0].organisationUnits = selectedOrgUnits.map((ounit) => {
        return { id: ounit };
      });
  ...
  return (
    <>
      <CustomMUIDialog open={true} maxWidth="md" fullWidth={true}>
        ...
        >
          Assign Organisation Units for Program{' '}
          {prgMetaData &&
            truncateString(prgMetaData.results?.programs[0].name, 40)}
        </CustomMUIDialogTitle>
        ...
        {orgUnitTreeRoot.length > 0 && (
          <DialogContent dividers style={{ padding: '1em 2em' }}>
              ...
              >
                ...
                >
                  <TextField
                    ...
                    helperText={
                      'Please Note: The Organisation Unit Tree is not automatically expanded after filtering'
                    }
                    variant="outlined"
                    inputRef={filterRef}
                    value={filterValue}
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <Tooltip title="Clear Filter" placement="left">
                            <span>
                              <IconButton
                                onClick={() => {
                                  resetSearch();
                  ...
                  <div style={{ marginTop: '10px' }}>
                    {' '}
                    {selectedOrgUnits.length} Organisation Units selected{' '}
                  </div>
                  ...
                  >
                    ...
                    <div style={{ flexDirection: 'row', marginTop: '1em' }}>
                      <FormControl
                        ...
                        style={{ width: '250px' }}
                      >
                        <InputLabel id={'organisation-unit-level-label'}>
                        ...
                        <Select
                          ...
                          onChange={(event) =>
                            setOrgUnitLevel(event.target.value)
                          }
                        >
                      ...
                      >
                        <Button
                          disabled={
                            !orgUnitLevel || loadingBulkAction || filterLoading
                          }
                          onClick={ouLevelAssignmentHandler}
                        >
                          SELECT
                        </Button>
                        <Button
                    ...
                    <div style={{ flexDirection: 'row', marginTop: '1em' }}>
                      <FormControl
                        ...
                        style={{ width: '250px' }}
                      >
                        <InputLabel id={'organisation-unit-Group-label'}>
                          Organisation Unit Group
                        </InputLabel>
                        <Select
                          labelId={'orgUnitGroupId'}
                          label={'Organisation Unit Group'}
                          onChange={(event) =>
                        ...
                        >
                          ...
                          {!ouMetadataLoading &&
                            ouMetadata.orgUnitGroups?.organisationUnitGroups.map(
                              function (ouGroup) {
                                return (
                                  <MenuItem value={ouGroup.id} key={ouGroup.id}>
                                    <em>{ouGroup.displayName}</em>
                                  </MenuItem>
                                );
                              }
                            )}
                      ...
                      >
                        <Button
                          disabled={
                            !orgUnitGroup || loadingBulkAction || filterLoading
                          }
                          onClick={ouGroupAssignmentHandler}
                        >
                          SELECT
                        </Button>
                        <Button
            ...
            {fetchErrors && (
              ...
              >
                <p>
                  Assign Organisation Units feature disabled due to the
                  following errors:
                </p>
                {fetchErrors}
              </Alert>
