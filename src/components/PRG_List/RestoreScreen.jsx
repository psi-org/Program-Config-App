...
import { BACKUPS_NAMESPACE } from '../../configs/Constants.jsx';
...
import RestoreItem from './RestoreItem.jsx';
        ...
        {!loading && (
          <>
            <CustomMUIDialogTitle
              ...
              id={'program_restore_dialog_title'}
            >
              {dataStore && restoreBackup
                ? 'Restoring ' + restoreBackup.name
                : 'Available Backups for Program ' +
                  truncateString(props.program.name, 40)}
            </CustomMUIDialogTitle>
            {!dataStore && (
              <Alert severity="error">
                No Backups found for the selected Program.
              </Alert>
            )}
                      ...
                      return (
                        <>
                          <RestoreItem
                            backup={backup}
                            key={index}
                            restoreHandler={onRestoreHandler}
                          />
                          {elements[index + 1] && (
            ...
            {dataStore && restoreBackup && (
              <RestoreOptions
                ...
                key={props.setRestoreProgramId}
                setRestoreProgramId={props.setRestoreProgramId}
