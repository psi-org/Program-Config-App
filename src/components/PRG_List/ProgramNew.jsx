...
const queryAvailablePrefix = {
  results: {
    ...
    params: ({ dePrefix, program }) => {
      const queryParams = {
        fields: ['id'],
        filters: [`${METADATA}:like:"dePrefix":"${dePrefix}"`],
      };
      if (program) {
...
const ProgramNew = (props) => {
  ...
  const { refetch: findTEAttributes } = useDataQuery(queryTEAttributes, {
    lazy: true,
  });
  const { refetch: findCategoryCombos } = useDataQuery(queryCatCombos, {
    lazy: true,
  });
    ...
    if (value) {
      ...
      if (programTET !== '') {
        trackedEntityTypes
          ...
          .trackedEntityTypeAttributes.forEach((tea) => {
            const idx = programTEAs.selected.findIndex(
              (teaId) => teaId === tea.trackedEntityAttribute.id
            );
            if (idx > -1) {
              programTEAs.selected.splice(idx, 1);
            }
          });
      }
  ...
  const [useSections, setUseSections] = useState(
    props.data?.programSections?.length > 0
  );
  ...
  const updateAssignedAttributes = () => {
    ...
    programTEAs.selected.forEach((teaId) => {
      if (
        ...
        newAssignedAttributes.push(
          programTEAs.available.find(
            (tea) => tea.trackedEntityAttribute.id === teaId
          )
        );
      }
    });
  ...
  const handleChangeTEAs = (res) => {
    if (programTET !== '') {
      const TETAttributes = trackedEntityTypes.find(
        (tet) => tet.id === programTET.id
      ).trackedEntityTypeAttributes;

      if (
        TETAttributes.every((tea) =>
          res.selected.includes(tea.trackedEntityAttribute.id)
        )
      ) {
        ...
        programTEAs.selected = programTEAs.selected.filter(
          (teaId) =>
            TETAttributes.map((tea) => tea.trackedEntityAttribute.id).includes(
              teaId
            ) || res.selected.includes(teaId)
        );
        setProgramTEAs(DeepCopy(programTEAs));
        pushNotification(
  ...
  const fetchTrackerMetadata = () => {
    findTEAttributes().then((data) => {
      if (data?.results?.trackedEntityAttributes) {
        ...
        const availableTEAs = data.results.trackedEntityAttributes
          .filter(
            (tea) =>
              !existingTEAs
                .map((tea) => tea.trackedEntityAttribute.id)
                .includes(tea.id)
          )
          .map((tea) => ({
        ...
        if (props?.data?.trackedEntityType?.trackedEntityTypeAttributes) {
          teaOptions.selected = teaOptions.selected.concat(
            props.data.trackedEntityType.trackedEntityTypeAttributes
              .map((att) => att.trackedEntityAttribute.id)
              .filter((id) => !teaOptions.selected.includes(id))
          );
        }
        ...
        const assignedAtts =
          props.data && props.data.programSections
            ? teaOptions.selected
                .filter(
                  (teaId) =>
                    !props.data.programSections
        ...
        attributesFormSections.forEach((section) => {
          section.trackedEntityAttributes =
            section.trackedEntityAttributes.filter((tea) =>
              teaOptions.selected.includes(tea.id)
            );
        });
        setAttributesFormSections([...attributesFormSections]);
      }
    ...
    findCategoryCombos().then((ccdata) => {
      if (ccdata?.results?.categoryCombos) {
        setProgramCategoryCombos(
          [{ name: 'default', id: '' }].concat(ccdata.results.categoryCombos)
        );
      }
    });
  }, [pgrTypePCA]);
  ...
  useEffect(() => {
    setCreatePublicObjects(
      currentUser?.results?.authorities?.some((auth) =>
        ['F_PROGRAM_PUBLIC_ADD', 'ALL'].includes(auth)
      )
    );
  }, [currentUser]);
  ...
  useEffect(() => {
    if (props.data) {
      setProgramBackup(DeepCopy(props.data));
    }
  }, []);
  ...
  const submission = async () => {
    ...
    const useCompetency =
      pgrTypePCA === 'hnqis'
        ? h2SettingsRef.current.saveMetaData()?.useCompetencyClass === 'Yes'
        : undefined;
    ...
    const prgrm = props.data ? DeepCopy(props.data) : DeepCopy(Program);
    ...
    const auxstyle = {};
    if (programIcon) {
      auxstyle.icon = programIcon;
    }
    if (programColor) {
      auxstyle.color = programColor;
    }
    ...
    if (pgrTypePCA === 'hnqis') {
      ...
      if (!props.data) {
        ...
        actionPlanStage.programStageDataElements =
          actionPlanStage.programStageDataElements.map((psde) => {
            psde.programStage = {
              id: actionPlanId,
            };
            return psde;
        ...
        assessmentStage = prgrm.programStages.find((section) =>
          section.name.toLowerCase().includes('assessment')
        );
        const exclusionsDEs = [
          CRITICAL_STEPS,
          NON_CRITICAL_STEPS,
        ...
        excludedStageDEs = assessmentStage.programStageDataElements.filter(
          (elem) => !exclusionsDEs.includes(elem.dataElement.id)
        );
      }

      prgrm.programTrackedEntityAttributes = DeepCopy(
      ...
      if (!useCompetency) {
        ...
        if (props.data) {
          criticalSteps = prgrm.programStages
            ...
            .find(
              (section) =>
                section.dataElements.find((de) => de.id === CRITICAL_STEPS) ||
        ...
        if (!criticalSteps) {
          criticalSteps = prgrm.programStages
            .map((pStage) => pStage.programStageSections)
            .flat()
            .find((section) => section.name === 'Critical Steps Calculations');
        }
        criticalSteps.dataElements = [
          { id: CRITICAL_STEPS },
          { id: NON_CRITICAL_STEPS },
        ...
        prgrm.programTrackedEntityAttributes =
          prgrm.programTrackedEntityAttributes.filter(
            (ptea) => ptea.trackedEntityAttribute.id !== COMPETENCY_ATTRIBUTE
          );
      ...
      if (assessmentStage?.programStageDataElements.length == 0 || props.data) {
        assessmentStage.programStageDataElements = excludedStageDEs.concat(
          criticalSteps.dataElements.map((de, index) => ({
      ...
      if (pgrTypePCA === 'tracker') {
        ...
        prgrm.programTrackedEntityAttributes = programTEAs.selected.map(
          (teaId, idx) => {
            const t = programTEAs.available.find(
              (tea) => tea.trackedEntityAttribute.id === teaId
            );
            t.sortOrder = idx;
            return t;
  ...
  function createOrUpdateMetaData(attributeValues) {
    ...
    if (metaDataArray.length > 0) {
      ...
      metaData_value.saveVersion = import.meta.env.DHIS2_APP_VERSION;
      ...
      val.saveVersion = import.meta.env.DHIS2_APP_VERSION;
            ...
            <Stepper alternativeLabel nonLinear activeStep={activeStep}>
              ...
              {(pgrTypePCA === 'tracker' || pgrTypePCA === 'event') && (
                <Step
                  ...
                  container={containerRef.current}
                >
                  <StepLabel onClick={() => changeStep(1)}>
                    {pgrTypePCA === 'tracker' ? 'Tracker' : 'Event'} Program
                    Settings
                  </StepLabel>
                </Step>
              )}
              {pgrTypePCA === 'tracker' && (
                <Step
                  ...
                  container={containerRef.current}
                >
                  <StepLabel onClick={() => changeStep(2)}>
              ...
              <div style={{ display: activeStep === 0 ? 'inherit' : 'none' }}>
                ...
                >
                  ...
                  <TextField
                    ...
                    autoComplete="off"
                    inputProps={{
                      maxLength: MAX_PROGRAM_NAME_LENGTH /*- dePrefix.length*/,
                    }}
                    value={programName}
                  ...
                  <TextField
                    ...
                    autoComplete="off"
                    inputProps={{
                      maxLength: MAX_SHORT_NAME_LENGTH /*- dePrefix.length*/,
                    }}
              ...
              >
                ...
                >
                  <Autocomplete
                    ...
                    options={programCategoryCombos?.map((pcc) => ({
                      label: pcc.name,
                      id: pcc.id,
                    }))}
                    sx={{ width: '100%' }}
                  ...
                  {!createPublicObjects && (
                    ...
                    >
                      ...
                      <label style={{ marginLeft: '0.5em' }}>
                        TEA Sections are considered Public Objects. Currently,
                        your User does not have permission to create Public
                        Objects.
                      </label>
                    </div>
                  )}
