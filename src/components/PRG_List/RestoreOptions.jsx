...
import { DeepCopy, parseErrorsJoin } from '../../utils/Utils.jsx';
...
const RestoreOptions = (props) => {
  ...
  const [checkedState, setCheckedState] = useState(new Array(11).fill(false));
  ...
  const onCheckBoxHandler = (position) => {
    const changeState = !(
      [0, 6, 8].includes(position) && childrenChecked(position)
    );
    if (changeState) {
  ...
  const runBackup = (request, metadataPayload, dryRun) => {
    request.mutate({ data: metadataPayload }).then((response) => {
      setIsLoading(false);
  ...
  const restoreHandler = (dryRun) => {
    const hnqis_attributes = [
      ORGANISATION_UNIT_ATTRIBUTE,
      HEALTH_AREA_ATTRIBUTE,
      ASSESSMENT_DATE_ATTRIBUTE,
    ];
    const hnqis_elements = [
      ACTION_PLAN_RESPONSIBLE,
      ACTION_PLAN_DUE_DATE,
      ACTION_PLAN_ACTION,
      NON_CRITICAL_STEPS,
    ...
    if (checkedState[0]) {
      ...
      if (ouOption === 'keepOUnits') {
        metadataPayload.programs[0].organisationUnits = DeepCopy(
          progMetaData.results?.programs[0]?.organisationUnits
        );
      }
      metadataPayload.attributes = DeepCopy(props.backup.metadata.attributes);
      if (checkedState[9]) {
        //ProgramRule CHecked
        metadataPayload.programRules = DeepCopy(
          props.backup.metadata.programRules
        );
        metadataPayload.programRuleActions = DeepCopy(
          props.backup.metadata.programRuleActions
        );
        metadataPayload.programRuleVariables = DeepCopy(
          props.backup.metadata.programRuleVariables
        );
      }
      if (checkedState[5]) {
        //Program Tracked Entity Attributes
        metadataPayload.programTrackedEntityAttributes = DeepCopy(
          props.backup.metadata.programTrackedEntityAttributes
        );
      }
      if (checkedState[3]) {
        //TETypes Checked
        metadataPayload.trackedEntityTypes = DeepCopy(
          filterComponent(
            props.backup.metadata.trackedEntityTypes,
            hnqis_tet_exception
          )
        );
      }
      if (checkedState[4]) {
        ...
        metadataPayload.trackedEntityAttributes = DeepCopy(
          filterComponent(
            props.backup.metadata.trackedEntityAttributes,
            hnqis_attributes
          )
        );
      }
      ...
      if (checkedState[10]) {
        //Program Indicators
        metadataPayload.programIndicators = DeepCopy(
          props.backup.metadata.programIndicators
        );
      }
      if (sharingOption === 'keepSharing') {
        metadataPayload.programs[0].sharing = DeepCopy(
          progMetaData.results?.programs[0]?.sharing
        );
        if (metadataPayload.attributes?.length > 0) {
          metadataPayload.attributes.forEach((attribute) => {
            const match = progMetaData.results?.attributes?.filter(
              (param) => param.id === attribute.id
            );
        ...
        if (checkedState[3]) {
          ...
          metadataPayload.trackedEntityTypes.forEach((tei) => {
            const match = progMetaData.results?.trackedEntityTypes?.filter(
              (param) => param.id === tei.id
            );
            if (match && match.length > 0) {
        ...
        if (checkedState[4]) {
          ...
          metadataPayload.trackedEntityAttributes.forEach((tea) => {
            const match = progMetaData.results?.trackedEntityAttributes?.filter(
              (param) => param.id === tea.id
            );
            if (match && match.length > 0) {
        ...
        if (checkedState[1]) {
          ...
          metadataPayload.optionSets?.forEach((optionSet) => {
            const match = progMetaData.results?.optionSets?.filter(
              (param) => param.id === optionSet.id
            );
            if (match && match.length > 0) {
        ...
        if (checkedState[10]) {
          metadataPayload.programIndicators?.forEach((pi) => {
            const match = progMetaData.results?.programIndicators?.filter(
              (param) => param.id === pi.id
            );
            if (match && match.length > 0) {
      ...
      if (checkedState[6]) {
        //Program Stage
        metadataPayload.programStages = DeepCopy(
          props.backup.metadata.programStages
        );
        if (sharingOption === 'keepSharing') {
          metadataPayload?.programStages?.forEach((ps) => {
            const match = progMetaData.results?.programStages.filter(
              (param) => param.id === ps.id
            );
            if (match && match.length > 0) {
        ...
        if (checkedState[7]) {
          ...
          metadataPayload.programStageSections = DeepCopy(
            props.backup.metadata.programStageSections
          );
        }
        if (checkedState[8]) {
          //Program Stage Data EElement
          metadataPayload.programStageDataElements = DeepCopy(
            props.backup.metadata.programStageDataElements
          );
          if (checkedState[9]) {
            //Data Element
            metadataPayload.dataElements = DeepCopy(
              filterComponent(
                props.backup.metadata.dataElements,
                hnqis_elements
              )
            );
            if (sharingOption === 'keepSharing') {
              metadataPayload?.dataElements?.forEach((de) => {
                const match = progMetaData.results?.dataElements?.filter(
                  (param) => param.id === de.id
                );
                if (match && match.length > 0) {
      ...
      if (checkedState[9] && !dryRun) {
        const deleteList = {};
        deleteList['programRules'] = progMetaData.results?.programRules
          ? arraySubset(progMetaData.results?.programRules, 'id')
          : [];
        deleteList['programRuleVariables'] = progMetaData.results
          ?.programRuleVariables
          ? arraySubset(progMetaData.results?.programRuleVariables, 'id')
          : [];

        deletePRnPVRequest.mutate({ data: deleteList }).then((response) => {
          if (response.status !== 'OK') {
            props.setNotification({
              message: `Something went wrong while Restoring Program. Please try again later.`,
              severity: 'error',
  ...
  const programStageDEChildren = (
    <Box sx={{ display: 'flex', flexDirection: 'column', ml: 3 }}>
      <FormControlLabel
        key={9}
        label="Data Elements + Program Rules & Variables"
        control={
          <Checkbox
            checked={checkedState[9]}
            onChange={() => onCheckBoxHandler(9)}
          />
        }
      />
    </Box>
  ...
  const programChildren = (
    <Box sx={{ display: 'flex', flexDirection: 'column', ml: 3 }}>
      <FormControlLabel
        key={1}
        label="Options (Options + Option Sets)"
        control={
          <Checkbox
            checked={checkedState[1]}
            onChange={() => onCheckBoxHandler(1)}
          />
        }
      />
      <FormControlLabel
  ...
  return (
    <>
      <DialogContent dividers style={{ padding: '1em 2em' }}>
        {validationResult && (
          <>
            <FormLabel focused={false} style={{ fontWeight: 'bold' }}>
              Import Summary
            </FormLabel>
            <TableContainer
        ...
        <FormControl style={{ width: '100%' }}>
          ...
          >
            <FormLabel
              id="elements-row-radio-buttons-group-label"
              focused={false}
        ...
        <FormControl>
          <FormLabel
            id="orgUnit-row-radio-buttons-group-label"
            focused={false}
            style={{ fontWeight: 'bold' }}
          >
            Organisation Units:{' '}
          ...
          <RadioGroup
            row
            aria-labelledby="orgUnit-row-radio-buttons-group-label"
            name="restore_OrganisationUnit"
            defaultValue="keepOUnits"
          >
            <FormControlLabel
              value="keepOUnits"
              control={<Radio />}
              label="Keep current settings"
              onChange={onOUOptionChangeHandler}
            />
            <FormControlLabel
              value="overwriteOUnits"
              control={<Radio />}
              label="Overwrite with backed up configuration"
              onChange={onOUOptionChangeHandler}
            />
          </RadioGroup>
        </FormControl>
        ...
        <FormControl>
          <FormLabel
            id="sharing-row-radio-buttons-group-label"
            focused={false}
            style={{ fontWeight: 'bold' }}
          >
            Sharing Settings:{' '}
          ...
          <RadioGroup
            row
            aria-labelledby="sharing-row-radio-buttons-group-label"
            name="restore_Sharing"
            defaultValue="keepSharing"
          >
            <FormControlLabel
              value="keepSharing"
              control={<Radio />}
              label="Keep current settings"
              onChange={onSharingChangeHandler}
            />
            <FormControlLabel
              value="overwriteSharing"
              control={<Radio />}
              label="Overwrite with backed up configuration "
              onChange={onSharingChangeHandler}
            />
      ...
      >
        <div style={{ display: 'flex', gap: '0.75em' }}>
          <Button onClick={hideFormHandler} color={'error'} variant="outlined">
            Cancel
          </Button>
          <LoadingButton
            onClick={() => restoreHandler(true)}
        ...
        <label style={{ fontSize: '0.8em', paddingTop: '0.8em' }}>
          <em>
