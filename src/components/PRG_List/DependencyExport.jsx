...
const DependencyExport = ({ program, programType, setExportProgramId }) => {
  ...
  const prgSectionsQuery = useDataQuery(queryProgramSections, {
    variables: { program },
  });
  ...
  const [attributeSettings, setAttributeSettings] = useState(
    DeepCopy(JSON_ATTRIBUTE_SETTINGS)
  );
  const [selectedPreset, setSelectedPreset] = useState('local');

  // JSON Customization
  const [jsonCustomizationEnabled, setJsonCustomizationEnabled] =
    useState(false);
  ...
  if (prgMetadata && legends && prgSections && !documentReady) {
    ...
    legends.forEach((legend) => {
      if (
        prgMetadata.dataElements?.find((de) =>
          de.legendSets?.find((l) => l.id == legend.id)
        )
      ) {
    ...
    const tabsList = Object.keys(keyTree).map((key) => ({
      key,
      selected: true,
    }));
  ...
  const downloadFile = () => {
    ...
    attributeSettings?.forEach((setting) => {
      if (setting.selected) {
        globalAttributesToRemove = globalAttributesToRemove.concat(
          setting.affects
        );
      }
    });
    ...
    if (selectedPreset === 'h2External') {
      ...
      for (let i = (attributes?.length || 0) - 1; i >= 0; i--) {
        if (!H2_ATTRIBUTES_TO_KEEP.includes(attributes[i].id)) {
          attributes.splice(
            attributes.findIndex((a) => a.id === attributes[i].id),
            1
          );
        }
      }
      ...
      de?.forEach((element) => {
        ...
        for (let i = attrValues.length - 1; i >= 0; i--) {
          if (!H2_ATTRIBUTES_TO_KEEP.includes(attrValues[i].attribute.id)) {
            attrValues.splice(
              attrValues.findIndex(
                (a) => a.attribute.id === attrValues[i].attribute.id
              ),
              1
            );
          }
    ...
    const blob = new Blob([JSON.stringify(cleanMetadata)], {
      type: 'text/json',
    });
    ...
    if (downloadOriginal) {
      const blob2 = new Blob([JSON.stringify(programMetadata)], {
        type: 'text/json',
      });
        ...
        <DialogContent dividers style={{ padding: '1em 2em' }}>
          {!documentReady && !exportError && (
            <div style={{ display: 'flex', alignItems: 'center' }}>
              ...
              <p>Exporting metadata and creating file...</p>
            </div>
          )}
          ...
          {exportError && (
            <div style={{ lineHeight: '1.5em' }}>
              ...
              <p>
                {exportError?.httpStatus +
                  ' - Status ' +
                  exportError?.httpStatusCode}
              </p>
                ...
                <div style={{ lineHeight: '1.5em' }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <p>
                        You can now download the metadata related to the program
                        by clicking &quot;Download&quot;.
                      </p>
                      <p>
                        <strong>Program:</strong>{' '}
                        <em>{prgMetadata.programs[0].name}</em>
                      </p>
                      <FormControlLabel
                        control={
                          <Switch
                            ...
                            onChange={() =>
                              setJsonCustomizationEnabled(
                                !jsonCustomizationEnabled
                              )
                            }
                          />
                        }
                    ...
                    >
                      <SelectOptions
                        ...
                        styles={{ width: '100%' }}
                        helperText={
                          ['h2External'].includes(selectedPreset)
                            ? 'This option performs special modifications to the file'
                            : ''
                        }
                      />
                      <p style={{ color: '#2c6693' }}>
                        <br />
                        <strong>NOTE: </strong>Keep in mind that any{' '}
                        <em>Option Groups</em> or <em>Option Group Sets</em>{' '}
                        related to the program are <strong>NOT</strong> included
                ...
                <Collapse in={jsonCustomizationEnabled} timeout={1000}>
                          ...
                          <FormControlLabel
                            ...
                            control={
                              <Checkbox
                                ...
                                onChange={() =>
                                  changeSelectedHeader(key.key, !key.selected)
                                }
                              />
                            }
                  ...
                  <Accordion style={{ marginTop: '1em' }}>
                    ...
                    <AccordionDetails>
                      ...
                      >
                        {attributeSettings.map((elem, index) => (
                          <FormControlLabel
                            key={index}
                            control={
                              <Switch
                                ...
                                onChange={() =>
                                  changeAttributeSettings(index, !elem.selected)
                                }
                                name=""
                              />
                            }
                      ...
                      >
                        <label>
                          <strong>NOTE: </strong>These settings are prioritized
                          over what&apos;s selected below.
                        </label>
                      </div>
                    </AccordionDetails>
                  </Accordion>
                  ...
                  >
                    ...
                    <ButtonGroup variant="outlined">
                      <Button
                        onClick={() =>
                          changeSelected(true, jsonKeyTree, setJsonKeyTree)
                        }
                      >
                        Select All
                      </Button>
                      <Button
                        onClick={() =>
                          changeSelected(false, jsonKeyTree, setJsonKeyTree)
                        }
                      >
                        Deselect All
                      </Button>
                  ...
                  >
                    {tabValue !== 0 &&
                      jsonKeyTree[tabValue].map((elem, index) => (
                        <FormControlLabel
                          key={index}
                          control={
                            <Checkbox
                              checked={elem.selected}
                              onChange={() =>
                                handleKeyCheckboxChange(tabValue, index)
                              }
                            />
                          }
                          label={elem.key}
