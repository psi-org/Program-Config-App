    ...
    params: ({ token, pageSize, page }) => {
      const paramsObject = {
        ...
        fields: [
          ...
          'trackedEntityType[id,name,trackedEntityTypeAttributes[trackedEntityAttribute[id]]]',
          'style',
          ...
          'programStages[id, name, allowGenerateNextVisit, formType, generatedByEnrollmentDate, sortOrder, hideDueDate, enableUserAssignment, minDaysFromStart, openAfterEnrollment, repeatable, remindCompleted, displayGenerateEventBox, validationStrategy, autoGenerateEvent, blockEntryForm, program, sharing, programStageDataElements, translations, attributeValues, programStageSections[*]]',
          'access',
          'withoutRegistration',
        ],
        filter: [],
...
const ProgramList = () => {
  ...
  const [pageSize, setPageSize] = useState(
    Number(localStorage.getItem('programsListPageSize')) || 10
  );
  const [currentPage, setCurrentPage] = useState(
    Number(localStorage.getItem('programsListPage')) || 1
  );
  ...
  const [filterValue, setFilterValue] = useState(
    localStorage.getItem('programsListFilter') || ''
  );
  ...
  const shareProgram = (program, prgType) => {
    const prg = data.results?.programs?.filter((p) => {
      return p.id === program;
    });
  ...
  const { loading, error, data, refetch } = useDataQuery(query, {
    variables: { token: filterValue, pageSize, page: currentPage },
  });
  ...
  return (
    <div>
      ...
      <div>
        ...
        <div
          style={{ display: 'flex', alignItems: 'center', padding: '0 1.2em' }}
        >
          <TextField
            margin="dense"
            id="name"
      ...
      {data.results?.programs?.length > 0 && (
        <div
          className="wrapper"
          style={{ padding: '0 1.2em', marginTop: '0.5em' }}
        >
          <Pagination
