...
const H2Convert = ({
        ...
        (tab) =>
          (newDataElementsList = newDataElementsList.concat(
            tab.dataElements.map((de) => {
              const programStageDataElement = h1StageDataElements.find(
      ...
      setScoresData(
        program.programStages
          ...
          .programStageDataElements.map((psde) => ({
            ...
            metadata: psde.dataElement.attributeValues.reduce((meta, att) => {
              meta[att.attribute.id] = att.value;
              return meta;
            }, {}),
          }))
          .filter(
            (score) =>
              score.dataElement.attributeValues.find(
                (att) => att.attribute.id === DE_TYPE_ATTRIBUTE
              )?.value === '93' &&
              score.dataElement.attributeValues.find(
          ...
          .sort((a, b) => {
            const aStruct = a.dataElement.attributeValues
              .find((att) => att.attribute.id === COMPOSITIVE_SCORE_ATTRIBUTE)
              .value.split('.');
  ...
  const convertProgram = async () => {
        ...
        if (hideGroup && hideType) {
          if (hideType === 'PARENT') {
            questionHideGroups[hideGroup] =
              de.programStageDataElement.dataElement.id;
          }
        }
      });
    });
      ...
      section.dataElements = section.dataElements
        ...
        .map((de, deIndex) => {
          ...
          if (de.metadata[COMPOSITIVE_SCORE_ATTRIBUTE]) {
            // FEEDBACK ORDER
            if (
              de.metadata[COMPOSITIVE_SCORE_ATTRIBUTE] !== compositiveScoreOrder
            ) {
          ...
          let parentValue = optionsMap.find(
            (option) =>
              option.id ===
              de.metadata[QUESTION_PARENT_OPTIONS_ATTRIBUTE]?.split(',')[0]
          )?.code;
          ...
          const pcaMetadataIndex = dataElement.attributeValues.findIndex(
    ...
    if (uidPool && prgTypeId) {
      ...
      prgrm.code = programOld.code
        ? '[HNQIS2] ' + programOld.code.slice(0, 41)
        : undefined;
      ...
      programStages.forEach((ps) => {
        ps.programStageDataElements.forEach(
          (psde) =>
            (psde.dataElement.id =
              labelIDMapping[psde.dataElement.id] || psde.dataElement.id)
        );
      });
  ...
  return (
    <>
        ...
        >
          ...
          {!loading && (
            <>
              ...
              <Accordion style={{ margin: '1.5em 0' }}>
                <AccordionSummary
                  ...
                  <Accordion style={{ margin: '0.5em 0' }} key={i}>
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <em>{`${section.dataElements.length} Data Element${
                      ...
                      {section.dataElements.map((de, key) => (
                        ...
                        >
                          ...
                          <div>
                            ...
                            {de.programStageDataElement.compulsory && (
                              <span
                                style={{
                                  color: 'red',
                                }}
      ...
      <CustomMUIDialog open={statusModal} maxWidth="sm" fullWidth={true}>
        <DialogTitle id="alert-dialog-title">Conversion Status</DialogTitle>
        <DialogContent
          dividers
          style={{
            padding: '1em 2em',
        ...
        >
          ...
          {conversionError && (
            <div>
              <p>The process could not be completed, see details below:</p>
