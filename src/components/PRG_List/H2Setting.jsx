...
import { AGG_TYPES_H2_PI, BUILD_VERSION } from '../../configs/Constants.jsx';
...
const H2Setting = forwardRef((props, ref) => {
  ...
  const {
    loading: ouLevelLoading,
    refetch: ouLevelRefetch,
    error: noOULevelFound,
  } = useDataQuery(ouUnitQuery, { variables: { id: id } });
  ...
  const [aggregationType, setAggregationType] = useState(
    props.pcaMetadata?.programIndicatorsAggType || 'AVERAGE'
  );
  const [useCompetency, setUseCompetency] = useState(
    props.pcaMetadata?.useCompetencyClass === 'Yes'
  );
  const [createAndroidAnalytics, setCreateAndroidAnalytics] = useState(
    props.pcaMetadata?.createAndroidAnalytics === 'Yes'
  );
  const [teiDownloadAmount, setTeiDownloadAmount] = useState(
    props.pcaMetadata?.teiDownloadAmount || 5
  );
  ...
  useEffect(() => {
    const checkAndResetValues = (key, setter) => {
      if (
        !ouMetadata.orgUnitLevels.organisationUnitLevels.some(
          (olevel) => olevel.id === key
        )
      ) {
        setter('');
      }
  ...
  useImperativeHandle(ref, () => ({
    handleFormValidation() {
      ...
      validationErrors.orgUnitRoot =
        selectedOrgUnits.length === 0 ? 'This field is required' : undefined;
      ...
      if (teiDownloadAmount === '') {
        ...
        validationErrors.teiDownloadAmount =
          'This field must contain a value between 0 and 500.';
        response = false;
      } else {
        validationErrors.teiDownloadAmount = undefined;
      }
    ...
    <>
      ...
      {orgUnitTreeRootLoaded && orgUnitTreeRoot.length === 0 && (
        <Alert severity="error">
          Your user has not been assigned to any Organisation Units. Please
      ...
      {orgUnitTreeRoot.length > 0 && (
        ...
        >
          ...
          >
            ...
            <FormControl
              variant={'standard'}
              error={validationErrors.orgUnitRoot !== undefined}
              style={{
                overflow: 'auto',
            ...
            >
              ...
              >
                <legend style={{ color: 'rgba(0, 0, 0, 0.6)' }}>
                  Generated Dashboard Settings
                </legend>
                <SelectOptions
                  useError={validationErrors.ouTableRow !== undefined}
                  helperText={validationErrors.ouTableRow}
                  label={'Org Unit Level for the Dashboard Visualizations (*)'}
                  items={ouLevelOptions}
                  handler={ouTableRowChange}
                  styles={{ width: '100%' }}
                  value={ouTableRow}
                  defaultOption={'Select Org Unit Level'}
                />
                <SelectOptions
                  useError={validationErrors.ouMapPolygon !== undefined}
                  helperText={validationErrors.ouMapPolygon}
                  label={'Org Unit Level for the Dashboard Maps (*)'}
                  items={ouLevelOptions}
              ...
              >
                ...
                />
                <SelectOptions
                  useError={validationErrors.aggregationType !== undefined}
                  helperText={validationErrors.aggregationType}
                  label={'Aggregation Type for Program Indicators (*)'}
                  items={AGG_TYPES_H2_PI}
                  handler={aggregationTypeChange}
                  styles={{ width: '100%' }}
                  value={aggregationType}
                  defaultOption={'Select Aggregation Type'}
                />
              </fieldset>
            </div>
