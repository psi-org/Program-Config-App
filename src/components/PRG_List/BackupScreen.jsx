...
const BackupScreen = (props) => {
  ...
  const [programName, setProgramName] = useState(
    truncateString(props.program.name, 30, false) +
      '_' +
      formatDate(new Date(), '_', '_')
  );
  ...
  const { loading: loadingMetadata, data: metaData } = useDataQuery(
    programMetadata,
    {
      onError: (err) => {
  ...
  const programBackupHandler = () => {
    ...
    if (
      ...
      versionInput.current.value.trim() === ''
    ) {
      nameInput.current.value.trim() === ''
    ...
    const backupToDatastore = !dsData?.results
      ? dsCreateRequest
      : dsUpdateRequest;
    backupToDatastore.mutate({ data: dsBackups }).then((response) => {
  ...
  const processMetadata = (metadata) => {
    ...
    if (metaData) {
      ...
      metadata.dataElements = DeepCopy(
        filterComponent(metadata.dataElements, hnqis_elements)
      );

      metadata.trackedEntityTypes = DeepCopy(
        filterComponent(metadata?.trackedEntityTypes, hnqis_tet_exception)
      );
      ...
      metadata.trackedEntityAttributes = DeepCopy(
        filterComponent(metadata.trackedEntityAttributes, hnqis_attributes)
      );
    ...
    <>
      <CustomMUIDialog open={true} maxWidth="md" fullWidth={true}>
        {(loadingMetadata || processing) && (
          <Box sx={{ display: 'inline-flex', margin: '50px' }}>
            <CircularProgress />
          </Box>
        )}
        {!(loadingMetadata || processing) && (
          <>
            <CustomMUIDialogTitle
              ...
              id={'program_backup_dialog_title'}
            >
              Create Backup for Program{' '}
              {truncateString(metaData.results?.programs[0].name, 40)}
            </CustomMUIDialogTitle>
            <DialogContent dividers style={{ padding: '1em 2em' }}>
              <TextField
                ...
                helperText={
                  validationError ? 'Please provide a Backup Name' : ''
                }
              ...
              <TextField
                ...
                helperText={
                  versionValidationError
                    ? 'Please provide a valid Version identifier'
                    : ''
                }
