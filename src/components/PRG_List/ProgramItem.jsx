...
const ProgramItem = ({
  ...
  const pcaMetadata = JSON.parse(
    program.attributeValues.find((av) => av.attribute.id === METADATA)?.value ||
      '{}'
  );
  const typeTag = {
  ...
  const requiredUpgradeBadge =
    ...
    !versionGTE(pcaMetadata.buildVersion, REQUIRED_H2_PROGRAM_BUILD_VERSION) ? (
      <Tooltip
        title={`IMPORTANT UPGRADE REQUIRED. This program was built using an older version of the PCA with deprecated logic and will not work properly. Please 'Set Up Program' again to fix this issue.`}
      >
        <SyncProblemIcon
          sx={{ fontSize: 35, color: '#FE3636' }}
          style={{ marginRight: '0.3em', cursor: 'pointer' }}
        />
      </Tooltip>
    ) : null;
      ...
      >
        ...
        >
          <div className="ml_item-desc" style={{ width: '3.2em' }}>
            <div
              style={{
                backgroundColor: program.style?.color || '#e0e0e0',
                width: '3em',
                height: '3em',
                minWidth: '3em',
                minHeight: '3em',
            ...
            >
              <img
                src={`${
                  ...
                  program.style?.icon || 'clinical_fe_outline'
                }/icon.svg`}
                style={{
                  width: '100%',
                  height: 'auto',
                  ...
                  filter: `brightness(0) invert(${
                    tinycolor(program.style?.color || '#e0e0e0').isDark()
                      ? 1
                      : 0
                  })`,
                  objectFit: 'cover',
        ...
        <div className="ml_item-desc">
          ...
          {programType !== 'HNQIS' &&
            ...
            !versionIsValid(
              pcaMetadata.buildVersion,
              import.meta.env.DHIS2_APP_VERSION,
              import.meta.env.DHIS2_APP_VERSION
            ) && (
              <Tooltip
          ...
          {programType !== 'HNQIS' && !pcaMetadata.buildVersion && (
            <Tooltip title={"This Program's logic hasn't been built yet."}>
              <NewReleasesIcon
                sx={{ fontSize: 30, color: '#FE9136' }}
                style={{ marginRight: '0.3em', cursor: 'pointer' }}
              />
            </Tooltip>
          )}
          {programType !== 'HNQIS' &&
            !(pcaMetadata.dePrefix && pcaMetadata.dePrefix !== '') && (
              <Tooltip
                title={`A Data Element Prefix is not defined for this Program. Please edit the Program Settings to add a Prefix.`}
              >
                <WarningIcon
                  sx={{ fontSize: 30, color: '#FEBB36' }}
                  style={{ marginRight: '0.3em', cursor: 'pointer' }}
          ...
          <div
            style={{
              backgroundColor:
                typeTag[programType]?.color ||
                typeTag[!program.withoutRegistration ? 'Tracker' : 'Event']
                  .color,
              width: '85px',
              display: 'flex',
          ...
          >
            {typeTag[programType]?.text ||
              typeTag[!program.withoutRegistration ? 'Tracker' : 'Event'].text}
          </div>
        </div>
