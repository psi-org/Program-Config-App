...
import {
  ...
  getProgramQuery,
  mergeWithPriority,
  parseErrorsSaveMetadata,
  extractMetadataPermissionsAllLevels,
  setPCAMetadata,
  padValue,
...
const buildRemovedDE = (de) => {
  ...
  de.name =
    de.name
      .replaceAll(/\d{7}\[X\]/g, '')
      .slice(0, MAX_FORM_NAME_LENGTH - suffix.length) + suffix;
...
const processStageData = ({
  ...
  let psdeSortOrder = 1;
  const stageIndex =
    stagesList?.findIndex((stage) => stage.id === programStage.id) || 0;
  importedSections.forEach((section, secIdx) => {
    if (
      section.isBasicForm ||
      section.formType === 'DEFAULT' ||
      section.id === 'basic-form'
    ) {
      isBasicForm = true;
    }
    if (section.importStatus == 'new') {
      section.id = uidPool.shift();
    }

    section.dataElements.forEach((dataElement, deIdx) => {
      const DE_metadata = JSON.parse(
        dataElement.attributeValues?.find(
          (att) => att.attribute.id === METADATA
        )?.value || '{}'
      );

      const newVarName = hnqisMode
        ? `_S${padValue(secIdx + 1, '00')}Q${padValue(deIdx + 1, '000')}`
        : `_PS${padValue(stageIndex + 1, '00')}_S${padValue(
            secIdx + 1,
            '00'
          )}E${padValue(deIdx + 1, '000')}`;
      const prefixOption =
        section.id === 'basic-form' ? programStage.id : section.id;
      const newCode = `${
        programMetadata.dePrefix || prefixOption
      }${newVarName}`;
      ...
      if (hnqisMode) {
        formName =
          DE_metadata.elemType == 'label'
            ? DE_metadata.labelFormName
            : dataElement.formName;

        formName = formName?.replaceAll(' [C]', '') || '';
        if (DE_metadata.isCritical == 'Yes') {
          formName += ' [C]';
        }
        DE_metadata.elemType == 'label'
          ? (DE_metadata.labelFormName = formName)
          : (dataElement.formName = formName);
      } else {
        formName = dataElement.formName;
      }

      const name = (newCode + '_' + formName).slice(0, MAX_FORM_NAME_LENGTH);
      const shortName = (newCode + '_' + formName).slice(
        0,
        MAX_SHORT_NAME_LENGTH
      );
      ...
      if (!hnqisMode) {
        ['isCritical', 'labelFormName'].forEach(
          (key) => delete DE_metadata[key]
        );
      }
  ...
  const importedDataElements = importedSections
    .map((sec) => sec.dataElements)
    .flat();
  importedSections.map((section) => {
    section.dataElements.map((de) => {
      const attributeValues = de.attributeValues;
      if (de.parentQuestion) {
        const metadataIndex = de.attributeValues.findIndex(
          (att) => att.attribute.id == METADATA
        );

        const metadata = JSON.parse(
          de.attributeValues[metadataIndex]?.value || '{}'
        );

        const parentId = importedDataElements.find(
          (ide) => ide.parentName == de.parentQuestion
        )?.id;
  ...
  const removed =
    (removedItems && removedItems.length) > 0
      ? removedItems.map((de) => buildRemovedDE(de))
      : [];
  ...
  programStage.programStageSections = !isBasicForm
    ? [].concat(
        importedSections,
        hnqisMode ? importedScores : [],
        hnqisMode ? criticalSection : []
      )
    : [];
  ...
  const programMetadataIdx = programPayload.attributeValues.findIndex(
    (att) => att.attribute.id === METADATA
  );
  const new_programMetadata = JSON.parse(
    programPayload.attributeValues.find((att) => att.attribute.id == METADATA)
      ?.value || '{}'
  );
  ...
  new_programMetadata.saveVersion = import.meta.env.DHIS2_APP_VERSION;
...
const processProgramData = ({
  ...
  const programPCAMetadata = JSON.parse(
    programPayload.attributeValues?.find((att) => att.attribute.id === METADATA)
      ?.value || '{}'
  );
  ...
  importedStages.forEach((programStage) => {
    const currentExistingStage = stagesList.find(
      (stage) => stage.id === programStage.id
    );
    ...
    const { tempMetadata, metadata } = processStageData({
      ...
      removedItems:
        importResults.stages.find((stage) => stage.id === programStage.id)
          ?.dataElements?.removedItems || [],
      programMetadata,
      stagesList: stagesList,
      originalStageDataElements: currentExistingStage.programStageDataElements,
      stageSharings: extractMetadataPermissionsAllLevels(
        DeepCopy(currentExistingStage.sharing)
      ),
      programPayload,
    });
  ...
  if (programPayload.withoutRegistration === false) {
    ...
    importedTEAs.forEach((teaSection, index) => {
      const programTrackedEntityAttributes =
        teaSection.trackedEntityAttributes.map((ptea) => {
      ...
      if (teaSection.id !== 'basic-form') {
        teaConfigurations.programSections.push({
          ...
          trackedEntityAttributes: programTrackedEntityAttributes.map(
            (ptea) => ({ id: ptea.trackedEntityAttribute.id })
          ),
        });
      }

      teaConfigurations.programTrackedEntityAttributes =
        teaConfigurations.programTrackedEntityAttributes.concat(
          programTrackedEntityAttributes
        );
  ...
  //* Result Object
  return {
    tempMetadata: { dataElements: tempMetadataDEs },
...
const SaveMetadata = (props) => {
  ...
  // Get Program payload
  const { refetch: getProgramPayload } = useDataQuery(queryProgram, {
    lazy: true,
    variables: { id: props.programId },
  });

  // Get Ids for new Data Elements
  const idsQuery = useDataQuery(queryId, {
    variables: { n: props.newObjectsQtty },
  });
  ...
  useEffect(() => {
    if (uidPool && programPayload && !completed && !metadataRequest.called) {
      ...
      const removedItems =
        props.saveType === 'stage'
          ? props.removedItems || []
          : props.removedItems?.dataElements || [];
      const { tempMetadata, metadata, teaConfigurations } =
        props.saveType === 'stage'
          ? processStageData({
              uidPool,
              ...
              stageSharings: extractMetadataPermissionsAllLevels(
                DeepCopy(props.programStage.sharing)
              ),
      ...
      metadataRequest.mutate({ data: tempMetadata }).then((response) => {
        if (response.status != 'OK') {
          ...
          metadataRequest
            .mutate({ data: programPayloadBackup })
            .then((response2) => {
              if (response.status != 'OK') {
  ...
  return (
    <CustomMUIDialog open={true} maxWidth="sm" fullWidth={true}>
      <CustomMUIDialogTitle
        ...
        onClose={() => props.setSavingMetadata(false)}
      >
      ...
      <DialogContent dividers style={{ padding: '1em 2em' }}>
        <NoticeBox title="Saving content" error={errorStatus}>
          ...
          {successStatus && (
            <div>
              <p>
                <strong>
                  Process completed!{' '}
                  {props.hnqisMode && '"Set up program" button is now enabled'}
                </strong>
              </p>
              {importFlag && (
                <p style={{ marginTop: '1em' }}>
                  <strong>Please Note:</strong>
                  As you imported Metadata from an Excel Template, make sure you
                  download an updated Template before making more changes.
                </p>
              )}
          ...
          {errorStatus && (
            <div>
              <p>
                <strong>Process ended with errors </strong>
              </p>
              <p>
                <strong>Please check the Errors Summary </strong>
              </p>
            </div>
          )}
        </NoticeBox>
