            ...
            <AccordionDetails>
                {
                    programRules.map((rule, i) => (
                        <Accordion style={{ marginTop: '1em' }} key={i} disabled={rule.actions.length === 0}>
                            ...
                            <AccordionDetails>
                                <List dense={true}>
                                    {rule.actions.map((action, index) => formatAction(action, index))}
                                </List>
                            </AccordionDetails>
                        </Accordion>
                    ))
...
const ProgramRulesList = props => {
    ...
    const { data: programRulesTrig, refetch: refetchProgramRulesTrig } = useDataQuery(queryRules, { lazy: true, variables: { prVars: props.variables } });
    const programRulesTriggered = programRulesTrig?.results?.programRules.filter(pr => pr.program.id === props.program).map(pr => ({
        id: pr.id,
        name: pr.name,
    ...
    const programRulesActing = Object.values(
        programRuleActions?.results?.programRuleActions?.reduce((rules, action) => {
            if (!rules[action.programRule.id]) {
                rules[action.programRule.id] = { id: action.programRule.id, name: action.programRule.name, condition: action.programRule.condition, actions: [action] };
            } else {
    ...
    useEffect(() => {
        ...
        if (!props.compositiveIndicator) {
            refetchProgramRulesTrig({ variables: { program: props.program, prVars: props.variables } })
        }
        // Get Score Program Rules
        if (props.compositiveIndicator) {
            refetchScoreActions({ variables: { program: props.program, compositiveIndicator: props.compositiveIndicator } })
        }
    }, [])
    ...
    return (
        <Box sx={{ width: '100%', marginTop: "1em", bgcolor: '#eeeeee', padding: "1em" }}>
            ...
            {programRulesActing && tab === 0 && ProgramRulesGroup("Program Rules acting over this Data Element", programRulesActing)}
