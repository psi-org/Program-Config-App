...
const IconPicker = ({ parentIcon, setParentIcon }) => {
  ...
  const filterIcons = () => {
    let newList = iconsList.filter((icon) =>
      iconFilter === '' ? true : icon.key.includes(iconFilter)
    );
    newList = newList.filter((icon) =>
      iconFilterText === '' ? true : icon.key.includes(iconFilterText)
    );
        ...
        <CustomMUIDialog open={true} maxWidth="lg" fullWidth={true}>
          ...
          >
            ...
            >
              ...
              >
                <Button
                  variant={iconFilter === '' ? 'outlined' : 'text'}
                ...
                <Button
                  variant={iconFilter === 'positive' ? 'outlined' : 'text'}
                  onClick={() => setIconFilter('positive')}
                >
                  {' '}
                  Positive{' '}
                </Button>
                <Button
                  variant={iconFilter === 'negative' ? 'outlined' : 'text'}
                  onClick={() => setIconFilter('negative')}
                >
                  {' '}
                  Negative{' '}
                </Button>
                <Button
                  variant={iconFilter === 'outline' ? 'outlined' : 'text'}
                  onClick={() => setIconFilter('outline')}
                >
                  {' '}
                  Outline{' '}
                </Button>
