...
const filterEntities = (entities, keyword, type, selectUserOrGroup) => {
  return entities
    ...
    .map((entity) => (
      <ListItem key={entity.id} onClick={() => selectUserOrGroup(type, entity)}>
...
const Suggestions = memo(
  ({ usersNGroups, keyword, setSearch, addEntity, inputRef }) => {
    ...
    useEffect(() => {
      ...
      const nUsers = filterEntities(
        usersNGroups?.userData?.users || [],
        keyVal,
        'userAccesses',
        selectUserOrGroup
      );
      const nUserGroups = filterEntities(
        usersNGroups?.userGroupData?.userGroups || [],
        keyVal,
        'userGroupAccesses',
        selectUserOrGroup
      );
