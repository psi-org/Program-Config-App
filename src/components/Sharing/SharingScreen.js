...
const SharingScreen = ({ element, id, setSharingProgramId }) => {
    ...
    let payload, usersNGroups, entityType;
    let entity = {};
    ...
    const setEntity = (type, ent) => {
        setUsrGrp(ent.displayName);
        entityType = type;
        entity.id = ent.id;
        entity.name = ent.name;
        entity.displayName = ent.displayName;
        console.log("entyt; ", entity);
    }
    ...
    const deleteUserPermission = (type, id) => {
        switch (type) {
            case 'publicAccess':
                ...
                break;
            case 'externalAccess':
                payload.meta.allowExternalAccess = false;
                payload.object.externalAccess = false;
                break;
            case 'userAccesses':
    ...
    function availableUserGroups() {
        ...
        obj.userAccesses.forEach((ua)=> {
            entities.userData.users.splice(entities.userData.users.findIndex(u => { return u.id = ua.id}), 1);
        });
        obj.userGroupAccesses.forEach((uga) => {
            entities.userGroupData.userGroups.splice(entities.userGroupData.userGroups.findIndex(ug=> { return ug.id = uga.id}), 1);
        });
        return entities;
    }
                    ...
                    <div style={{ fontWeight: 400, padding: "16px", backgroundColor: 'rgb(245,245,245)', display: "flex", flexDirection: 'column', justifyContent: "center" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "row", flex: "1 1 0"}}>
                            <div style={{ display: "inline-block", position: "relative", width: "100%", backgroundColor: "white", boxShadow: 'rgb(204,204,204) 2px 2px 2px', padding: "0px 16px", marginRight: "16px"}}>
                                <input type={"text"} autoComplete={"off"} id={"userNGroup"} onChange={loadSuggestions} value={usrGrp} style={{ appearance: "textfield", padding: "0px", position: "relative", border: "none", outline: "none", backgroundColor: 'rgba(0,0,0,0)', color: 'rgba(0,0,0,0.87)', cursor: "inherit", opacity: 1, height: "100%", width: "100%"}} placeholder={"Enter Names"}/>
                            </div>
                            {search && <Suggestions usersNGroups={usersNGroups} keyword={search} setSearch={setSearch} setEntity={setEntity}/>}
                            <div id={'newPermission'} style={{ paddingLeft: "12px", paddingRight: "6px"}} onClick={()=>{toggle(); }}>
                                { userPermissionState() }
                            </div>
