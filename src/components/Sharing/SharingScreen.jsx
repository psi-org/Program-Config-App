...
import SharingOptions from './SharingOptions.jsx';
...
const psDataElementAccess = {
  des: {
    ...
    params: {
      paging: false,
      fields: [
        'programStages[programStageDataElements[dataElement[id,name,access]]]',
      ],
    },
  },
};
...
const btnOptions = [
  ...
  'Apply to Program & Program Stages',
  'Apply to Program, Program Stages & Data Elements',
];
...
const SharingScreen = ({
  ...
  const { loading, error, data } = useDataQuery(sharingQuery, {
    variables: { element: element, id: id },
  });
  const {
    loading: entityLoading,
    data: entities,
    error: entityErrors,
  } = useDataQuery(entitiesQuery);
  ...
  const { loading: prgDELoading, data: prgDEData } = useDataQuery(
    psDataElementAccess,
    { variables: { id: id } }
  );
  const { data: vData } = useDataQuery(visualizationQuery, {
    variables: { id: id },
  });
  ...
  const exclusionDataElements = [
    ...
    'DIoqtxbSJIL',
    'nswci5V4j0d',
  ]; //Excluding HNQIS2 specific data elements from sharing update

  useEffect(() => {
    if (readOnly) {
      setRestrictions((prev) => [
        ...prev,
        "You don't have access to update this program",
      ]);
    }
  }, [readOnly]);
  ...
  useEffect(() => {
    if (!entityLoading && entityErrors) {
      setRestrictions((prev) => [
        ...prev,
        "You don't have access to Users List",
      ]);
    }
    ...
    if (!prgDELoading && prgDEData) {
      ...
      prgStages.forEach((ps) => {
        ps.programStageDataElements.forEach((de) => {
          if (
            !de.dataElement.access.update &&
            !exclusionDataElements.includes(de.dataElement.id)
          ) {
            restrictedDataElements.push({
      ...
      if (restrictedDataElements.length > 0) {
        ...
        setRestrictions((prev) => [
          ...prev,
          "You don't have update access to Data Elements Listed below",
        ]);
  ...
  if (!metadataLoading && prgMetaData) {
    ...
    const psde = metadata.programStages
      ?.map((ps) =>
        ps.programStageDataElements.map((psde) => psde.dataElement.id)
      )
      ?.flat();
    metadata.dataElements = metadata.dataElements?.filter((de) =>
      psde.includes(de.id)
    );
  }
  ...
  const userPermissionState = () => {
    return (
      <IconButton color="inherit" style={{ marginRight: '1em' }}>
        {usrPermission[1] === 'w' ? (
          <EditIcon />
        ) : usrPermission[0] === 'r' && usrPermission[1] !== 'w' ? (
          <ViewIcon />
        ) : (
          <BlockIcon />
        )}
  ...
  function availableUserGroups() {
    ...
    if (e !== null) {
      obj.userAccesses.forEach((ua) => {
        e.userData.users.splice(
          e.userData.users.findIndex((u) => {
            return u.id === ua.id;
          }),
          1
        );
      ...
      obj.userGroupAccesses.forEach((uga) => {
        e.userGroupData.userGroups.splice(
          e.userGroupData.userGroups.findIndex((ug) => {
            return ug.id === uga.id;
          }),
          1
        );
  ...
  const deleteUserPermission = (type, id) => {
    switch (type) {
      ...
      case 'userGroupAccesses':
        var userGroupIndex = data.results.object.userGroupAccesses.findIndex(
          (userGroup) => {
  ...
  const updateUserPermission = (type, id, permission) => {
    switch (type) {
      ...
      case 'userAccesses':
        ...
        break;
      case 'userGroupAccesses':
        var userGroupIndex = payload.object.userGroupAccesses.findIndex(
          (userGroup) => {
            return userGroup.id === id;
          }
        );
  ...
  const apply = (level) => {
    ...
    const programType = programTypeId
      ? metadata.programs[0].attributeValues?.filter((av) => {
    ...
    switch (level) {
      case 1:
        ...
        break;
      case 2:
        payloadMetadata.dataElements =
          metadata.dataElements?.filter(
            (de) => !exclusionsDEs.includes(de.id)
          ) || [];
        payloadMetadata.programStages = metadata.programStages;
        break;
      default:
    ...
    metadataRequest.mutate({ data: payloadMetadata }).then((response) => {
  ...
  const applySharing = (elements, meta) => {
    ...
    elements?.forEach((element) => {
      ...
      if (!exclusionDataElements.includes(element.id)) {
        if (meta === 'dataElements' && overwrite) {
          //Overwrite all the permission for dataElements if checked
          ...
          element.sharing.public =
            meta === 'dataElements' || meta === 'programIndicators'
              ? dePermission(payload.object.publicAccess)
          ...
          payload.object.userAccesses.forEach((user) => {
            if (Object.hasOwn(element.sharing.users, user.id) && overwrite) {
              element.sharing.users[user.id].access =
                meta === 'dataElements' || meta === 'programIndicators'
                  ? dePermission(user.access)
                  : user.access; //update permission if user exist
            } else {
          ...
          payload.object.userGroupAccesses.forEach((userGroup) => {
            if (
              Object.hasOwn(element.sharing.userGroups, userGroup.id) &&
              overwrite
            ) {
              element.sharing.userGroups[userGroup.id].access =
                meta === 'dataElements' || meta === 'programIndicators'
                  ? dePermission(userGroup.access)
                  : userGroup.access;
            } else {
  ...
  const deSharing = (obj) => {
    ...
    obj.userGroupAccesses.forEach((userGroup) => {
      temp.userGroups[userGroup.id] = {
        id: userGroup.id,
        access: dePermission(userGroup.access),
      };
    });
    return temp;
  };
  ...
  const addHNQISElement4Sharing = (hnqisElements) => {
    ...
    if (dashboardsDQ.data?.results.dashboards) {
      addElement(
        'dashboard',
        dashboardsDQ.data?.results.dashboards,
        hnqisElements
      );
    }
    ...
    if (eventReportDQ.data.results?.eventReports) {
      addElement(
        'eventReport',
        eventReportDQ.data.results?.eventReports,
        hnqisElements
      );
    }
  };
  ...
  return (
    <>
      {runAdditionalSharing &&
        additionalElements.length > 0 &&
        additionalElements.map(function (v) {
          return (
            <ObjectSharing
              key={v.id}
              id={v.id}
        ...
        <ModalContent>
          ...
          {content === 'form' && (
            <div>
              ...
              >
                {truncateString(data.results?.object.displayName, 40)}
              </h2>
                ...
                {data.results?.object.userAccesses.map(function (userAccess) {
                  return (
                    <SharingItem
                      ...
                      permission={userAccess.access}
                      updatePermission={updateUserPermission}
                      deleteUserPermission={deleteUserPermission}
                    />
                  );
                })}
                {data.results?.object.userGroupAccesses.map(function (
                  userGroupAccess
                ) {
                  return (
                    <SharingItem
                      key={userGroupAccess.id}
                      type={'userGroupAccesses'}
                      element={{
                        id: userGroupAccess.id,
                        displayName: userGroupAccess.displayName,
                      }}
                      permission={userGroupAccess.access}
                      updatePermission={updateUserPermission}
                      deleteUserPermission={deleteUserPermission}
                    />
                  );
              ...
              >
                <div
                  style={{ color: 'rgb(129, 129, 129)', paddingBottom: '8px' }}
                >
                ...
                >
                  ...
                  {optionOpen && (
                    <SharingOptions
                      permission={usrPermission.split('')}
          ...
          <ModalActions>
            <ButtonStrip end>
              ...
              >
                <Button
                  onClick={handleClick}
                  disabled={readOnly || !!fetchErrors || !metadata}
                >
                  {btnOptions[selectedIndex]}
                </Button>
                <Button
                  size="small"
