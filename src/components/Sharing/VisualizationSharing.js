...
const VisualizationSharing = ({ id, sharing }) => {
  ...
  const { loading, data } = useDataQuery(sharingQuery, {
    variables: { element: 'visualization', id: id },
  });
  ...
  if (!loading) {
    ...
    sharing.object.userAccesses.forEach((user) => {
      payload.object.userAccesses.push({
        id: user.id,
        access: dePermission(user.access),
      });
    });
    sharing.object.userGroupAccesses.forEach((userGroup) => {
      payload.object.userGroupAccesses.push({
        id: userGroup.id,
