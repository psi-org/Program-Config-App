...
const AttributesModal = ({ programId, onClose }) => {
  ...
  // * Data Queries
  const { refetch: programRefetch } = useDataQuery(queryProgram, {
    variables: { programId },
    lazy: true,
  });
  const { refetch: teaRefetch } = useDataQuery(queryTEA, { lazy: true });
  const [updateMetadata, { loading: isUpdating }] = useDataMutation(
    createMutation,
    { lazy: true }
  );
  ...
  const loadProgramData = () => {
    teaRefetch().then((teaResults) => {
      programRefetch({ programId }).then((programResults) => {
        ...
        const entityAtts =
          programResults.results.trackedEntityType.trackedEntityTypeAttributes.map(
        ...
        const existingTEAs =
          programResults.results.programTrackedEntityAttributes.map((tea) => ({
        ...
        const availableTEAs = teaResults.results.trackedEntityAttributes
          .filter(
            (tea) =>
              !existingTEAs
        ...
        const assignedAtts = teaOptions.selected
          .filter(
            (teaId) =>
              !programResults.results.programSections
                .map((section) =>
  ...
  const handleTransferChange = (e) => {
    if (
      e.selected.length === 1 &&
      !teaOptions.selected.includes(e.selected[0])
    ) {
      // ADDED
      const teaId = e.selected[0];
      teaOptions.selected.push(teaId);

      assignedAttributes.push(
        teaOptions.available.find(
      ...
      // REMOVED
      const removedId = teaOptions.selected.filter(
        (teaId) => !e.selected.includes(teaId)
      )[0];
      ...
      attributesFormSections.forEach((section) => {
        const teaIdx = section.trackedEntityAttributes.findIndex(
          (tea) => tea.id === removedId
        );
        if (teaIdx > -1) {
  ...
  const onSubmit = () => {
    programRefetch({ programId }).then((programResults) => {
      ...
      program.programTrackedEntityAttributes = teaOptions.selected.map(
        (teaId, idx) => {
          const t = teaOptions.available.find(
            (tea) => tea.trackedEntityAttribute.id === teaId
          );
          t.sortOrder = idx;
          return t;
      ...
      program.programSections = data.programSections.map((section) => ({
        id: section.id,
      }));
    ...
    <>
      <CustomMUIDialog open={true} maxWidth="lg" fullWidth={true}>
        ...
        <CustomMUIDialogTitle
          ...
          onClose={() => setExitDisclaimer(true)}
        >
          ...
          <Stepper nonLinear activeStep={activeStep} orientation="vertical">
            <Step>
              <StepButton color="inherit" onClick={() => setActiveStep(0)}>
                Select Tracked Entity Attributes
              </StepButton>
              <StepContent>
                <Transfer
                  filterable
            ...
            <Step>
              <StepButton color="inherit" onClick={() => setActiveStep(1)}>
                Manage Tracked Entity Attributes Form
              </StepButton>
              <StepContent>
        ...
        <DialogActions style={{ padding: '1em' }}>
          <Button onClick={() => setExitDisclaimer(true)} color="error">
            {' '}
            Close{' '}
          </Button>
          <LoadingButton
    ...
    <CustomMUIDialog open={opened} maxWidth="sm" fullWidth={true}>
      ...
      <DialogContent dividers style={{ padding: '1em 2em' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <span>
            <strong>
              <em>Warning: All unsaved changes will be lost</em>
            </strong>
          </span>
        </div>
      </DialogContent>
