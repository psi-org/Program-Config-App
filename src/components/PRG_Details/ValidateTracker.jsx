...
const ValidateTracker = ({
  ...
  const [validationMessage, setValidationMessage] = useState(
    "Metadata validated. Please use the 'SAVE' button to persist your changes."
  );
  ...
  useEffect(() => {
    setValidationScope(
      importResults || buildProgramConfigurations(programMetadata)
    );
  }, []);
  ...
  useEffect(() => {
    if (validationScope && !save) {
      ...
      if (teas) {
        ...
        teas.forEach((section) => {
          ...
          section.trackedEntityAttributes.forEach((tea) => {
            excelRow += 1;
            const errorDetails = {
              title: tea?.trackedEntityAttribute?.name
                ? `${tea.trackedEntityAttribute.name} (Template row ${excelRow})`
                : 'Tracked Entity Attribute on Template row ' + excelRow,
              tagName: '[ Tracked Entity Attribute ]',
            };
            delete tea.errors;
            validateTEA(tea, teaList, errorDetails);
            if (tea.errors) {
              validationResults.teas.teas.push(tea);
            }
            if (tea.errors?.errors.length > 0) {
              errorCounts += tea.errors.errors.length;
            }
          });
        });
      }
      ...
      importedStages.forEach((stage) => {
        ...
        const dataElementsList = stage.importedSections
          ...
          section.dataElements.forEach((dataElement) => {
            ...
            const errorDetails = {
              title: dataElement?.name
                ? `${truncateString(
                    dataElement.name
                  )} (Template row ${excelRow})`
                : `Data Element on Template row ${excelRow}`,
            ...
            if (dataElement.errors) {
              validationResults.stages.dataElements.push(dataElement);
            }
            if (dataElement.errors?.errors.length > 0) {
              stage_errors += dataElement.errors.errors.length;
            }
          });
        });
      ...
      if (errorCounts === 0) {
        ...
        setValidationMessage(
          'Some Validation Errors occurred. Please check / fix the issues and upload again to continue.'
        );
  ...
  return (
    <CustomMUIDialog open={true} maxWidth="sm" fullWidth={true}>
      <CustomMUIDialogTitle
        ...
        onClose={() => setSavingMetadata(false)}
      >
        Settings Validation
      </CustomMUIDialogTitle>
      <DialogContent dividers style={{ padding: '1em 2em' }}>
        <NoticeBox
          ...
          title={
            processed
              ? 'Program configurations validated'
              : 'Validating program configurations'
          }
        >
      ...
      <DialogActions style={{ padding: '1em' }}>
        <Button
          color="error"
          disabled={!processed}
          onClick={() => setSavingMetadata(false)}
        >
          {' '}
          Close{' '}
        </Button>
        ...
        {save && validationScope && (
          <SaveMetadata
            ...
            importResults={
              importResults || { stages: [], teaSummary: { teas: [] } }
            }
