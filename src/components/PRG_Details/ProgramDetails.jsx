...
import ValidateTracker from './ValidateTracker.jsx';
...
const ProgramDetails = () => {
  ...
  useEffect(() => {
    if (notification) {
      setSnackSeverity(notification.severity);
    }
  }, [notification]);
  ...
  if (!program) {
    return (
      <NoticeBox title="Missing Program ID" error>
        No program ID was provided, please try again!{' '}
        <Link to="/">Go to programs list</Link>
      </NoticeBox>
    );
  }
  ...
  const buildProgramRulesTracker = () => {
    ...
    refetch({ program }).then((programConfig) => {
      ...
      const pcaMetadata = JSON.parse(
        programConfig.results?.attributeValues?.find(
      ...
      const n =
        (programStages?.reduce(
          (acu, cur) => acu + (cur.programStageDataElements?.length || 0),
          0
        ) || 0) * 4;
      ...
      idsQuery.refetch({ n }).then((uidData) => {
        if (uidData) {
          ...
          programStages.forEach((stage, stgIdx) => {
            ...
            const varNameRef = stageSections
              .map((sec) =>
            ...
            stageSections.forEach((section, secIdx) => {
              section.dataElements.forEach((dataElement, deIdx) => {
                programRuleVariables.push({
                  id: uidPool.shift(),
                  name: `_PS${padValue(stgIdx + 1, '00')}_S${padValue(
                    secIdx + 1,
                    '00'
          ...
          const metadata = {
            ...
            programRules: hideShowRules,
            programRuleActions: hideShowActions,
          };
          ...
          getDataStore().then((dataStoreResult) => {
            ...
            const fallbackRuleVariables =
              prvDQ.data.results.programRuleVariables.filter((prv) => {
                return prv.name[0] == '_';
              });
            ...
            const oldMetadata = {
              programRules:
                programRulesDel.length > 0 ? programRulesDel : undefined,
              programRuleVariables:
                programRuleVariablesDel.length > 0
  ...
  return (
    <div>
      ...
      <div
        className="wrapper"
        style={{ padding: '1em 1.2em 0', height: '75vh' }}
      >
        <div className="layout_prgms_stages">
          <div className="list-ml_item">
            ...
            {validationResults && (
              <Errors
                validationResults={validationResults.teas.sections
                  .concat(validationResults.teas.teas)
                  .concat(
                    validationResults.stages.sections.concat(
                      validationResults.stages.dataElements
                    )
                  )
                  .map((tea) => tea.errors)}
                key={'validationSec'}
            ...
            {importResults && (
              <Removed
                removedItems={(importResults.teaSummary
                  ? importResults.teaSummary.teas.removedItems
                  : []
                ).concat(
                  importResults.stages
                    .map((stage) => stage.dataElements.removedItems)
                    .flat()
                )}
                key={'removedSec'}
                tagText="Object(s)"
            ...
            <div style={{ width: '100%', padding: '0.5em 0' }}>
                ...
                .map((programStage) => {
                  return (
                    <StageItem
                      ...
                      importResults={importResults?.configurations?.importedStages?.find(
                        (result) => result.id === programStage.id
                      )}
                      key={programStage.id}
                      ...
                      editStatus={
                        newStage?.stage === programStage.id
                          ? newStage?.mode || ''
                          : ''
                      }
                      hnqisMode={hnqisMode}
              ...
              {data.results.programStages.length === 0 && (
                ...
                >
                  There are currently no Stages in this Program
                </div>
              )}
            </div>
          </div>
          ...
          <Importer
            ...
            programSpecificType={
              data.results.withoutRegistration
                ...
                : TEMPLATE_PROGRAM_TYPES.tracker
            }
            previous={{
              stages: DeepCopy(backupData.programData.programStages),
              programSections: DeepCopy(backupData.programData.programSections),
              teas: DeepCopy(
                backupData.programData.programTrackedEntityAttributes
              ),
            }}
