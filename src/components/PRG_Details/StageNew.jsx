...
const StageNew = (props) => {
  ...
  const [displayGenerateEventBox, setDisplayGenerateEventBox] = useState(
    props.data?.displayGenerateEventBox ?? true
  );
  const [autoGenerate, setAutogenerate] = useState(
    props.data?.autoGenerateEvent ?? true
  );
  const [openFormAfterEnroll, setOpenFormAfterEnroll] = useState(
    props.data?.openAfterEnrollment ?? false
  );
  const [reportDateToUse, setReportDateToUse] = useState(
    props.data?.reportDateToUse || ''
  );
  const [askCompleteProgram, setAskCompleteProgram] = useState(
    props.data?.remindCompleted ?? false
  );
  const [askCreateEvent, setAskCreateEvent] = useState(
    props.data?.allowGenerateNextVisit ?? false
  );
  ...
  const formDataIsValid = () => {
    ...
    if (String(scheduledDaysStart).trim() === '') {
      ...
      response = false;
      validationErrors.scheduledDaysStart =
        'This field must be equal or greater than 0';
    } else {
      validationErrors.scheduledDaysStart = undefined;
    }
    ...
    if (!metadataRequest.called && dataIsValid) {
      ...
      metadataRequest.mutate({ data: metadata }).then((response) => {
        if (response.status != 'OK') {
          ...
          props.setNotification({
            message: `Program Stage '${stage.name}' ${
              props.data?.id ? 'updated' : 'created'
            } successfully`,
            severity: 'success',
          });
          props.setShowStageForm(false);
          ...
          props.setNewStage({
            stage: stage.id,
            mode: props.data?.id ? 'updated' : 'created',
          });
        }
      });
    }
  ...
  function createOrUpdateMetaData(attributeValues) {
    ...
    if (metaDataArray.length > 0) {
      ...
      const val = { saveVersion: import.meta.env.DHIS2_APP_VERSION };
    ...
    <>
      <CustomMUIDialog open={true} maxWidth="md" fullWidth={true}>
        ...
        >
          ...
          >
            {props.data
              ? 'Edit Program Stage ' + props.data.name
              : 'Create New Program Stage in Program ' + props.programName}
          </div>
        </CustomMUIDialogTitle>
        <DialogContent dividers style={{ padding: '1em 2em' }}>
          ...
          >
            ...
            >
              <FormControlLabel
                control={
                  <Checkbox
                    checked={displayGenerateEventBox}
                    onChange={handleChangeDisplayGenerateEventBox}
                  />
                }
            ...
            {autoGenerate && (
              <>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={openFormAfterEnroll}
                      onChange={handleChangeOpenAfterEnroll}
                    />
                  }
          ...
          >
            <FormControlLabel
              control={
                <Checkbox
                  checked={askCompleteProgram}
                  onChange={handleChangeAskCompleteProgram}
                />
              }
            ...
            <FormControlLabel
              control={
                <Checkbox
                  checked={askCreateEvent}
                  onChange={handleChangeAskCreateEvent}
                />
              }
