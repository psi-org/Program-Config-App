...
const DraggableSection = ({
  ...
  if (stageSection.importStatus) {
    switch (stageSection.importStatus) {
      case 'new':
        sectionImportStatus = (
          <div style={{ minWidth: '4.5em', maxWidth: '4.5em' }}>
            <Tag positive>New</Tag>
          </div>
        );
        break;
      case 'update':
    ...
    sectionImportSummary = (
      <>
        <div style={newTagStyle}>{'New: ' + stageSection.newValues}</div>
  ...
  var classNames = stageSection.importStatus
    ? ' import_' + stageSection.importStatus
    : '';

  return (
    <Draggable
      key={stageSection.id || 'section' + index}
      draggableId={String(stageSection.id || index)}
      index={index}
      isDragDisabled={
        stageSection.importStatus != undefined ||
        DEActions.deToEdit !== '' ||
        !isSectionMode ||
        readOnly
      }
    >
      {(provided) => (
        ...
        >
          <div
            className={
              'ml_item section-header' +
              (openMenu ? ' section-selected' : '') +
              classNames
            }
            style={{
          ...
          >
            ...
            >
              ...
              <Tooltip
                title={stageSection.displayName}
              ...
              <div>
                {editStatus && (
                  <Chip
                    label={editStatus.mode.toUpperCase()}
                    color="success"
            ...
            <div className="ml_item-desc">
              <div style={tagStyle}>
                {stageSection.dataElements.length} Data Elements
              </div>
              {sectionImportSummary}
            </div>
            <div
                  ...
                  <Popper reference={ref} placement="bottom-end">
                    <FlyoutMenu>
                      <MenuItem
                        ...
                        onClick={() => {
                          toggle();
                          SectionActions.handleSectionEdit(index, undefined);
                        }}
                      />
                      <MenuItem
                        label="Create New Section Above"
                        icon={<UpIcon />}
                        onClick={() => {
                          toggle();
                          SectionActions.handleSectionEdit(undefined, index);
                        }}
                      />
                      <MenuItem
                        label="Create New Section Below"
                        icon={<DownIcon />}
                        onClick={() => {
                          toggle();
                          SectionActions.handleSectionEdit(
                            undefined,
                            index + 1
                          );
                        }}
                      />
                      <MenuItem
                        label="Add Data Element(s)"
                        icon={<AddCircleOutlineIcon />}
                        onClick={() => {
                          toggle();
                          DEActions.add(
                            stageSection.dataElements.length,
                            stageSection.id
                          );
                        }}
                      />
                      <MenuItem
                        ...
                        onClick={() => {
                          toggle();
                          setSectionToRemove(stageSection);
                        }}
                      />
                    </FlyoutMenu>
                  </Popper>
          ...
          <Droppable
            droppableId={stageSection.id || 'dropSec' + index}
            type="DATA_ELEMENT"
          >
            {(provided) => (
              <div
                ...
                {stageSection.dataElements.map((de, i) => {
                  return (
                    <DraggableDataElement
                      ...
                      dePrefix={dePrefix}
                      dataElement={de}
                      stageDE={stageDataElements.find(
                        (stageDE) => stageDE.dataElement.id === de.id
                      )}
                      DEActions={DEActions}
                      ...
                      key={de.id || i}
                      hnqisMode={hnqisMode}
                      deStatus={editStatus?.dataElements?.find(
          ...
          {showValidationMessage && (
            <ValidationMessages
              objects={[stageSection].concat(stageSection.dataElements)}
            ...
            <AlertDialogSlide
              ...
              preContent={
                <div>
                  <p style={{ marginBottom: '1.5em' }}>
                    <strong>CAUTION:</strong> All Data Elements associated to
                    this Section will also be removed from the Stage. Make sure
                    you move the Data Elements you want to keep into another
                    Section before deleting the current one.
                  </p>
                  <span>
                    <strong>Section to remove: </strong>
              ...
              actions={{
                primary: function () {
                  ...
                  SectionActions.remove(sectionToRemove);
                },
                secondary: function () {
                  setSectionToRemove(undefined);
                },
