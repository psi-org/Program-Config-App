...
export const readTemplateData = ({
  ...
  templateData.forEach((row, rowNum) => {
    switch (row[templateMap.structure]) {
      ...
      case 'Data Element':
        ...
        importedSections[isBasicForm ? 0 : sectionIndex].dataElements.push(
          mapImportedDE({
            data: row,
            programPrefix,
  ...
  if (mode === TEMPLATE_PROGRAM_TYPES.hnqis2) {
    ...
    importedScores.forEach((i_score) => {
      ...
      i_score.id == null
        ? importSummaryValues.scores.new++
        : importSummaryValues.scores.updated++;
    });

    // Removed Scores
    const removedScores = currentData.scoresSection.dataElements.filter(
      (de) => !importedScores.find((i_score) => i_score.id == de.id)
    );
