...
const dataElementsSearchQuery = {
  results: {
    ...
    params: ({ token, page }) => ({
      ...
      fields: [
        'id,name,shortName,code,description,domainType,formName,valueType,aggregationType,optionSetValue,optionSet[id,name],legendSet[id,name],legendSets,attributeValues,displayName, lastUpdated',
      ],
      filter: [
...
const DataElementManager = (props) => {
  ...
  const { loading: loadingSearch, refetch: search } = useDataQuery(
    dataElementsSearchQuery,
    {
      lazy: true,
      variables: { token: undefined, page: undefined },
    }
  );
  ...
  const checkSelectedDE = (model) => {
    setNewDataElements(
      model.map((id) => {
        ...
        if (!deObject) {
          deObject = rows.find((de) => de.id === id);
        }
        return deObject;
      })
    );
    ...
    <>
      <CustomMUIDialog open={true} maxWidth="xl" fullWidth={true}>
        <CustomMUIDialogTitle
          ...
          onClose={() => (tabValue == 0 ? hideForm() : setDialogStatus(true))}
        >
          ...
          <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
            ...
            >
              <Tab
                icon={<SearchIcon />}
                iconPosition="start"
                label="Select Existing Data Elements"
                {...a11yProps(1)}
              />
              <Tab
              ...
              <div style={{ height: 400, width: '100%' }}>
                <DataGrid
                  ...
                  isRowSelectable={(params) =>
                    !props.programStageDataElements.find(
                      (psde) => psde.dataElement.id === params.row.id
                    )
                  }
        ...
        >
          <Button
            onClick={() => (tabValue == 0 ? hideForm() : setDialogStatus(true))}
            color="error"
          >
            {' '}
            Close{' '}
          ...
          />
          <Button
            disabled={tabValue == 0 && newDataElements.length < 1}
            onClick={() => submission()}
            variant="contained"
            color="success"
