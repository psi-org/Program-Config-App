import {
  ProgramIndicatorTemplate,
  compLastSixMonthsByOUTable,
  compLastSixMonthsPie,
  compLastSixMonthsTable,
  ProgramIndicatorTemplateNoA,
  ProgramIndicatorTemplateGS,
  AverageScoreByDistrictByPivotTable,
  NumberOfAssessmentByPivotTable,
  AverageGlobalScoreByColumn,
  AssessmentByCompetencyByColumn,
  GlobalScoreByMap,
  LineListGlobalScore,
  dashboardsTemplate,
  dashVisualization,
  dashMap,
  dashEventReport,
} from '../../configs/AnalyticsTemplates.js';
import {
  FEEDBACK_ORDER,
  METADATA,
...
export const buildFeedbackTree = (dataElements, programRuleVariables) => {
    ...
    score.scoreLayout.forEach((lvl, index) => {
      if (!nav[lvl]) {
        ...
        if (
          !nav[lvl]?.content &&
          !nav[lvl].score &&
          index === score.scoreLayout.length - 1
        ) {
          nav[lvl].score = score;
        }
        nav = nav[lvl];
  ...
  const questionDEs = dataElements
    .filter(
      (de) =>
        JSON.parse(
          de.attributeValues.find((av) => av.attribute.id === METADATA)
    ...
    .sort((a, b) => {
      ...
      const semanticOrderA = foA
        .map((lvl) => new Array(3 - lvl.length + 1).join('0') + lvl)
        .join('');
      const semanticOrderB = foB
        .map((lvl) => new Array(3 - lvl.length + 1).join('0') + lvl)
        .join('');
  ...
  questionDEs.forEach((question) => {
    const scoreLayout = question.attributeValues
      .find((av) => av.attribute.id === FEEDBACK_ORDER)
      ?.value?.split('.');
    if (!scoreLayout) {
      return;
...
const getScorePR = ({ composite, branch, programId, stageId, uidPool }) => {
  ...
  if (currentOrder && composite.subLevels.length >= 0) {
    ...
    if (branch) {
      // Composite Score HAS scoring questions
      /**
...
export const checkScores = (scores) => {
  const compositeScores = scores.map(
    (score) =>
      score.attributeValues.find((att) => att.attribute.id == FEEDBACK_ORDER)
        ?.value
  );
  const duplicatedScores = compositeScores.filter(
    (composite, index) => compositeScores.indexOf(composite) !== index
  );
  return {
...
export const readQuestionComposites = (sections) => {
  ...
  sections.forEach((section) => {
    section.dataElements.forEach((de) => {
      const composite = de.attributeValues
        .find((att) => att.attribute.id == FEEDBACK_ORDER)
        ?.value?.split('.')
        .slice(0, -1);
      if (composite) {
        composite
          ...
          .forEach((v) => {
            ...
            if (!questionCompositeScores.includes(v)) {
              questionCompositeScores.push(v);
            }
          });
      }
    });
...
export const hideShowLogic = (hideShowGroup, programId, uidPool) => {
  ...
  Object.keys(hideShowGroup).forEach((parentCode) => {
    Object.keys(hideShowGroup[parentCode]).forEach((answer) => {
      ...
      const conditionValue = ['0', '1'].includes(String(answer))
        ? answer
        : `"${answer.replaceAll("'", '')}"`;
      ...
      if (pr_mfm.programRuleActions.length > 0) {
        hideShowRules.push(pr_mfm);
      }
    });
  });
...
export const buildProgramRules = ({
  ...
  sections.forEach((section, secIdx) => {
    section.dataElements.forEach((dataElement, deIdx) => {
      const order = dataElement.attributeValues.find(
        ...
        locateInTree(
          {
            ...
            prgVarName: `_S${padValue(secIdx + 1, '00')}Q${padValue(
              deIdx + 1,
              '000'
            )}`,
      ...
      if (
        metadata.parentQuestion !== undefined &&
        metadata.parentValue !== undefined
      ) {
        const parentQuestion = varNameRef.find(
          (de) => de.id === String(metadata.parentQuestion)
        ).varName;
        const parentValue = String(metadata.parentValue);

        !hideShowGroup[parentQuestion]
          ? (hideShowGroup[parentQuestion] = {})
          : undefined;
        !hideShowGroup[parentQuestion][parentValue]
          ? (hideShowGroup[parentQuestion][parentValue] = [])
          : undefined;
        !hideShowGroup[parentQuestion][parentValue].push({
          id: dataElement.id,
          mandatory: metadata.isCompulsory,
        });

        if (metadata.labelFormName) {
          let hsIdx = hideShowLabels.findIndex(
            (hs) => hs.parent == parentQuestion && hs.condition == parentValue
          );
          if (hsIdx == -1) {
            // CREATE
            hideShowLabels.push({
          ...
          hideShowLabels[hsIdx].actions.push({
            id: dataElement.id,
            text: metadata.labelFormName.replaceAll('"', "'"),
          });
        }
      } else if (metadata.labelFormName) {
        hideShowLabels[0].actions.push({
          id: dataElement.id,
          text: metadata.labelFormName.replaceAll('"', "'"),
        });
      }
  ...
  const criticalScore = buildCriticalScore({
    branch: scoreMap,
    stageId,
    programId,
    uidPool,
  });
  const nonCriticalScore = buildNonCriticalScore({
    branch: scoreMap,
    stageId,
    programId,
    uidPool,
  ...
  const { competencyRules, competencyActions } =
    useCompetencyClass == 'Yes'
      ? buildCompetencyRules(programId, stageId, uidPool)
      : { competencyRules: [], competencyActions: [] };
  ...
  const { hideShowRules, hideShowActions } = hideShowLogic(
    hideShowGroup,
    programId,
    uidPool
  );
  ...
  const { labelsRules, labelsActions } = labelsRulesLogic(
    hideShowLabels,
    programId,
    uidPool
  );
...
export const buildProgramIndicators = ({
  programId,
  programStage,
  scoreMap,
  uidPool,
  ...
  const mainScoreDataElement =
    scoreMap.numC == '' ? NON_CRITICAL_STEPS : CRITICAL_STEPS;
  ...
  const indicatorValues =
    useCompetency === 'Yes'
      ...
      : [
          {
            name: 'A',
            condition: `#{${programStage.id}.${mainScoreDataElement}} >= 80.0`,
          },
          {
            name: 'B',
            condition: `#{${programStage.id}.${mainScoreDataElement}} < 80.0 && #{${programStage.id}.${mainScoreDataElement}} >= 50.0`,
          },
          {
            name: 'C',
            condition: `#{${programStage.id}.${mainScoreDataElement}} < 50.0`,
          },
        ];
  ...
  AnalyticGS.aggregationType =
    PIAggregationType && PIAggregationType !== 'AVERAGE'
      ? PIAggregationType
      : 'AVERAGE';
...
export const buildH2BaseVisualizations = ({
  ...
  const nameComp =
    useCompetency === 'Yes' ? 'Competency Classes' : 'Quality of Care';
  ...
  chart1.name =
    programShortName + ' - ' + nameComp + ' Pie Chart - (Last 6 months)';
  ...
  table1.name =
    programShortName + ' - ' + nameComp + ' - (Last 6 months by Org Units)';
  ...
  avergeScorebyTable.name =
    programShortName + ' - Average Score by District from last 12 months';
  ...
  AverageGlobalScorebyColumn.name =
    programShortName +
    ' - Average Global Score by checklist from last 12 months';
  ...
  const AssessmentByCompetencybyColumn = DeepCopy(
    AssessmentByCompetencyByColumn
  );
  AssessmentByCompetencybyColumn.id = uidPool.shift();
  AssessmentByCompetencybyColumn.name =
    programShortName +
    ' - Number and Percentage of Assessments by Competency Class (last 4 quarters)';
  ...
  AssessmentByCompetencybyColumn.dataDimensionItems = dataDimensionItems.slice(
    0,
    3
  );
  ...
  NumberOfAssessmentbyTable.name =
    programShortName + ' - Number of Assessments by checklist (last 12 months)';
