...
import { MAX_SECTION_NAME_LENGTH } from '../../configs/Constants.jsx';
...
const SectionManager = ({
  ...
  const formDataIsValid = () => {
    return (
      sectionName.trim() !== '' && sectionName.length <= MAX_SECTION_NAME_LENGTH
    );
  };
  ...
  function submission() {
    ...
    if (formDataIsValid()) {
      ...
      notify(
        <span>
          Section {newSectionIndex !== undefined ? 'created' : 'edited'}!{' '}
          <strong>
            Remember to{' '}
      ...
      setAddedSection({
        index: sectionIndex ?? newSectionIndex,
        mode: newSectionIndex !== undefined ? 'created' : 'edited',
        dataElements: [],
      });
      hideForm();
    }
          ...
          helperText={
            sentForm
              ? sectionName.trim() === ''
                ...
                ? `The Section Name cannot exceed ${MAX_SECTION_NAME_LENGTH} characters long`
                : ''
              : ''
          }
          margin="normal"
