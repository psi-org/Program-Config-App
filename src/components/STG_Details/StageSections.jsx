...
const StageSections = ({ programStage, hnqisMode, readOnly }) => {
  ...
  const [originalProgramStageDataElements] = useState(
    programStage.programStageDataElements.reduce(
      (acu, cur) => acu.concat(cur),
      []
    )
  );
    ...
    isSectionMode
      ? [
          ...programStage.programStageSections.filter(
            (s) =>
              (s.name !== 'Scores' &&
                s.name !== 'Critical Steps Calculations') ||
              !hnqisMode
          ),
        ]
  ...
  const pushNotification = (content, severity = 'success') =>
  ...
  const updateDEValues = (dataElementId, sectionId, stageDataElement) => {
    ...
    const section_DE_idx = sections[sectionIdx].dataElements.findIndex(
      (de) => de.id === dataElementId
    );
    const stage_DE_idx = programStageDataElements.findIndex(
      (psde) => psde.dataElement.id === dataElementId
    );

    programStageDataElements[stage_DE_idx] = stageDataElement;
    sections[sectionIdx].dataElements[section_DE_idx] =
  ...
  const removeDE = (id, section) => {
    ...
    const sectionIdx = sections
      .find((s) => s.id === section)
      ?.dataElements.findIndex((de) => de.id === id);

    if (sectionIdx > -1 && psdeIdx > -1) {
      sections
        .find((s) => s.id === section)
        ?.dataElements.splice(sectionIdx, 1);
  ...
  const saveAdd = (params) => {
    ...
    const toBeAdded = params.newDataElements.map((de) => ({
    ...
    sections
      ...
      .dataElements.splice(
        params.deRef.index,
        0,
        ...dataElementObjects /* ...params.newDataElements */
      );
    const newProgramStageDataElements = programStageDataElements.concat(
      params.newDataElements
    );
  ...
  const SectionActions = {
    ...
    handleSectionEdit: (section = undefined, newSection = undefined) =>
      handleSectionEdit(section, newSection),
  ...
  const prDQ = useDataQuery(queryPR, {
    variables: { programId: programStage.program.id },
  });

  // Fetch Program Rule Variables from Program
  const prvDQ = useDataQuery(queryPRV, {
    variables: { programId: programStage.program.id },
  });

  // Fetch Program Indicators from Program
  const pIndDQ = useDataQuery(queryPIndicators, {
    variables: { programId: programStage.program.id },
  });
  ...
  // Fetch Visualizations from Program
  const mapsDQ = useDataQuery(queryMaps, {
    variables: { programId: programStage.program.id },
  });
  ...
  const { data: programAttributes, refetch: getProgramAttributes } =
    useDataQuery(queryPCAMetadata, {
      variables: { programId: programStage.program.id },
    });
  ...
  const getUIDs = () => {
    ...
    let n =
      ...
      visualizationsAmount +
      androidSettingsAmount;
  ...
  const commit = () => {
    setAddedSection(undefined);
    if (createMetadata.data && createMetadata.data.status) {
      delete createMetadata.data.status;
    }
  ...
  const buildAndroidSettings = (
    ...
    if (!settings.results.dhisVisualizations.home) {
      settings.results.dhisVisualizations.home = [];
    }

    settings.results.dhisVisualizations.home =
      settings.results.dhisVisualizations.home.filter(
        (setting) => setting.program !== programId
      );
  ...
  const importMetadataAndPersistAsync = async ({
    metadata,
    sendToDataStore,
    dataStoreData,
  }) => {
    ...
    try {
      ...
      await safeStep(1, async () => {
        ...
        const ouData = await setOuLevel({
          ouLevel: [pcaMetadata.ouLevelTable, pcaMetadata.ouLevelMap],
        });
        ...
        const visualizationLevel = levels.find(
          (l) => l.id === pcaMetadata.ouLevelTable
        );
      ...
      await safeStep(2, async () => {
        const { uniqueScores, compositeScores, duplicatedScores } = checkScores(
          scoresSection.dataElements
        );
        if (!uniqueScores) {
      ...
      const {
        ...
        const scoresMapping = scoresSection.dataElements.reduce(
          (acc, cur) => ({
            ...acc,
            [cur.attributeValues.find(
        ...
        const dataElements = programStage.programStageSections.reduce(
          (acc, cur) => acc.concat(cur.dataElements),
          []
        );
        ...
        const sendToDataStore = dataStoreResult?.results
          ? dataStoreUpdate
          : dataStoreCreate;
        let dataStoreData = dataStoreResult?.results
          ? dataStoreResult.results
          : {};
        const toDeleteReferences = DeepCopy(dataStoreData);
        dataStoreData = programRefereces;

        const fallbackRuleVariables =
          prvDQ.data.results.programRuleVariables.filter(
            (prv) => prv.name?.[0] === '_'
          );
        ...
        const programIndicatorsDel =
          toDeleteReferences?.programIndicators ||
          mapIdArray(pIndDQ.data.results.programIndicators);
        const visualizationsDel =
          toDeleteReferences?.visualizations ||
          mapIdArray(visualizationsDQ.data.results.visualizations);
        const eventReportsDel =
          toDeleteReferences?.eventReports ||
          mapIdArray(eventReportDQ.data.results.eventReports);
        const mapsDel =
          toDeleteReferences?.maps || mapIdArray(mapsDQ.data.results.maps);

        const oldMetadata = {
          programRules: programRulesDel.length ? programRulesDel : undefined,
          programRuleVariables: programRuleVariablesDel.length
            ? programRuleVariablesDel
            : undefined,
          programIndicators: programIndicatorsDel.length
            ? programIndicatorsDel
            : undefined,
          visualizations: visualizationsDel.length
            ? visualizationsDel
            : undefined,
      ...
      await safeStep(6, async () => {
        await importMetadataAndPersistAsync({
          metadata,
          sendToDataStore,
          dataStoreData,
        });
  ...
  const parseErrors = (e) => {
    const data = e.typeReports.map((tr) => {
      ...
      return tr.objectReports.map((or) =>
        or.errorReports.map((er) => ({
          type,
          uid: or.uid,
  ...
  const updateProgramBuildVersion = async (programId) => {
    ...
    res.results?.attributeValues?.forEach((av) => {
      if (av.attribute.id === METADATA) {
        ...
        pcaMetadata.buildVersion = import.meta.env.DHIS2_APP_VERSION;
        av.value = JSON.stringify(pcaMetadata);
      }
    });

    const response = await createMetadata.mutate({
      data: { programs: [res.results] },
    });
  ...
  if (
    hnqisMode &&
    !metadataLoading &&
    !versionGTE(hnqis2Metadata?.results?.version, H2_METADATA_VERSION)
  ) {
    return (
      <>
    ...
    <div className="cont_stage">
                          ...
                          <MenuList id="split-button-menu" autoFocusItem>
                            {optionsSetUp.map((option, index) => (
                              <MenuItem
                                ...
                                disabled={
                                  index ===
                                  1 /*&& (!allAuth || !(existingLocalAnalytics?.results?.visualizations.length > 0))*/
                                }
        ...
        <CustomMUIDialog open={true} maxWidth="sm" fullWidth={true}>
          ...
          <DialogContent dividers style={{ padding: '1em 2em' }}>
            ...
            <NoticeBox title="Please Note">
              <p>
                To enable In-app Analytics for this Program please contact your
      ...
      {hnqisMode && saveAndBuild && (
        <CustomMUIDialog open={true} maxWidth="sm" fullWidth={true}>
          <CustomMUIDialogTitle
            ...
            onClose={() => {
              if (
                saveAndBuild === 'Completed' ||
                createMetadata?.data?.status === 'ERROR'
              ) {
          ...
          <DialogContent dividers style={{ padding: '1em 2em' }}>
              ...
              <>
                <div className="progressItem">
                  ...
                  {!programSettingsError ? (
                    ...
                    <p style={{ maxWidth: '90%' }}>
                      Global analytics settings missing.
                    </p>
                  ) : programSettingsError === 2 ? (
                    <p style={{ maxWidth: '90%' }}>
                      Configured Organisation Unit Levels not found.
                    </p>
                  ) : (
                ...
                {programSettingsError === 2 && (
                  <NoticeBox title="Please do the following:">
                    <ol>
                      ...
                      <li>Open Program Menu</li>
                      <li>
                        Click Edit Program and make sure that the Organisation
                        Unit Levels are valid.
                      </li>
                    </ol>
                  </NoticeBox>
                )}
            ...
            {progressSteps > 6 && !programSettingsError && (
              <div className="progressItem">
                ...
                {progressSteps !== 7 &&
                  androidSettings &&
                  androidSettingsError && <IconCross24 color={'#d63031'} />}
                {progressSteps !== 7 && !androidSettings && (
                  <IconWarning24 color={'#ffbb00'} />
                )}
                {progressSteps !== 7 &&
                  androidSettings &&
                ...
                <p style={{ maxWidth: '90%' }}>
                  {programMetadata?.createAndroidAnalytics === 'Yes'
                  ...
                  {!androidSettings
                    ...
                    : androidSettingsError
                    ? '(Error: ' +
                      (androidSettingsError.httpStatus === 'Forbidden'
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="wrapper" style={{ overflow: 'auto' }}>
          <div className="layout_prgms_stages">
            ...
            {importResults &&
              (importResults.questions.removed > 0 ||
                importResults.scores.removed > 0) && (
                <Removed
                  removedItems={importResults.questions.removedItems.concat(
                    importResults.scores.removedItems
                  )}
                  key={'removedSec'}
                />
              )}
              ...
              (validationResults.sections.length > 0 ||
                ...
                <Errors
                  validationResults={validationResults.sections
                    .concat(
                      validationResults.questions.concat(
            ...
            {createMetadata.data && createMetadata.data.status == 'ERROR' && (
              <ErrorReports errors={parseErrors(createMetadata.data)} />
            )}
            {!programMetadata && (
              <div
                style={{
                    ...
                    {sections.map((pss, idx) => {
                      return (
                        <DraggableSection
                          ...
                          stageSection={pss}
                          editStatus={
                            addedSection?.index === idx && addedSection
                          }
            ...
            {hnqisMode && isSectionMode && (
              <>
                <CriticalCalculations
                  stageSection={criticalSection}
                  ikey={criticalSection?.id || 'crit'}
                />
                <Scores
                  stageSection={scoresSection}
                  key={scoresSection?.id || 'scores'}
                  program={programId}
                />
              </>
            )}
      ...
      {savingMetadata && (
        <ValidateMetadata
          ...
          newDEQty={
            importResults
              ? importResults.questions.new +
                importResults.scores.new +
                importResults.sections.new
              : 0
          }
          ...
          removedItems={
            importResults
              ? importResults.questions.removedItems.concat(
                  importResults.scores.removedItems
                )
              : removedElements
          }
