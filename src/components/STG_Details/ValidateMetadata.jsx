...
import { FEEDBACK_ORDER } from '../../configs/Constants.jsx';
import {
  validateFeedbacks,
  validateScores,
  validateQuestions,
  ...
  useEffect(() => {
    if (!save) {
      ...
      if (verifyProgramDetail(importResults, setValidationMessage)) {
        ...
        // CHECK FEEDBACK DATA
        if (hnqisMode) {
          const validateResults = validateFeedbacks(
            hnqisMode,
            importedSectionsV.concat(importedScoresV)
          );
        ...
        const dataElementsList = DeepCopy(
          importedSections.map((section) => section.dataElements).flat()
        );
        ...
        if (hnqisMode) {
          importedSectionsV.forEach((section) => {
            ...
            section.dataElements.forEach((dataElement) => {
              ...
              const errorDetails = {
                title:
                  dataElement.labelFormName ||
                  dataElement.formName ||
                  dataElement.name ||
                  'Element on Template row ' + excelRow,
                tagName: dataElement.labelFormName
                  ? '[ Label ]'
                  : '[ Question ]',
              };
              ...
              if (
                feedbacksErrors.find((fe) =>
                  fe.instance.elements.find((e) => e === dataElement.code)
                )
              ) {
                const deFeedBackOrder = dataElement.attributeValues.find(
                  (att) => att.attribute.id === FEEDBACK_ORDER
                )?.value;

                const deErrs = feedbacksErrors.find(
                  (fe) => fe.instance.feedbackOrder === deFeedBackOrder
                ).elementError.errorMsg;

                dataElement.errors
                  ? dataElement.errors.errors.push(deErrs)
                  : (dataElement.errors = {
          ...
          importedScoresV.dataElements.forEach((dataElement) => {
            ...
            const errorDetails = {
              title:
                dataElement.formName ||
                dataElement.name ||
                'Score on Template row ' + excelRow,
              tagName: '[ Score ]',
            };
            delete dataElement.errors;
      ...
      <DialogContent dividers style={{ padding: '1em 2em' }}>
        {hnqisMode && (
          <NoticeBox
            ...
            title={
              processed
                ? 'Sections and Scores Validated'
                : 'Validating Sections and Scores'
            }
          >
