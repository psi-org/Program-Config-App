...
const AttributesEditor = ({
  ...
  const handlePropChange = ({ prop, teaId, value }) => {
    teaOptions.available.find((tea) => tea.trackedEntityAttribute.id === teaId)[
      prop
    ] = value;
  ...
  const removeSection = (sectionIndex) => {
    attributesFormSections[sectionIndex].trackedEntityAttributes.forEach(
      (teaId) => {
        assignedAttributes.push(
          teaOptions.available.find(
            (tea) => tea.trackedEntityAttribute.id === teaId.id
          )
        );
      }
    );
    ...
    if (type === 'SECTION') {
      ...
      if (source.droppableId === destination.droppableId) {
        ...
        if (source.droppableId === 'ASSIGNED') {
          ...
          ].trackedEntityAttributes.splice(destination.index, 0, {
            id: i.trackedEntityAttribute.id,
          });
        } else {
          // ? From Section A to Section B
          const [i] = attributesFormSections[
            source.droppableId
          ].trackedEntityAttributes.splice(source.index, 1);
          attributesFormSections[
            destination.droppableId
          ].trackedEntityAttributes.splice(destination.index, 0, i);
        }
      }
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      {!useSections && (
        <BasicForm
          attributes={teaOptions.selected.map((teaId) =>
            teaOptions.available.find(
              (tea) => tea.trackedEntityAttribute.id === teaId
            )
          )}
