...
const toInnerQuery = (q) =>
  q?.results && typeof q.results === 'object' ? q.results : q;

const buildMultiQuery = (processList, prefix) => {
  const query = {};
  processList.forEach((proc, i) => {
    const raw =
      typeof proc.queryFunction === 'function'
        ? proc.queryFunction()
        : proc.queryFunction;
...
const App = () => {
  ...
  const pcaChecksQuery = useMemo(
    () => buildMultiQuery(checkProcessPCA, 'pca'),
    []
  );
  const h2ChecksQuery = useMemo(
    () => buildMultiQuery(checkProcessH2, 'h2'),
    []
  );

  const {
    data: pcaChecksData,
    loading: pcaChecksLoading,
    error: pcaChecksError,
  } = useDataQuery(pcaChecksQuery);

  const {
    data: h2ChecksData,
    loading: h2ChecksLoading,
    error: h2ChecksError,
  ...
  const pcaReady = useMemo(() => {
    ...
    return checkProcessPCA.every((proc, i) =>
      isComplete(pcaChecksData[`pca${i}`], proc)
    );
  ...
  const h2Ready = useMemo(() => {
    ...
    return checkProcessH2.every((proc, i) =>
      isComplete(h2ChecksData[`h2${i}`], proc)
    );
  ...
  const GateComponent = useMemo(() => {
    ...
    const needsUpdate =
      pcaMetaState.error || availableVersion < PCA_METADATA_VERSION;
