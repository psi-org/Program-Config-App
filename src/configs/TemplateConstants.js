...
export const conditionalError = {
  fill: { type: 'pattern', pattern: 'solid', bgColor: { argb: 'dc3545' } },
};
export const sectionHighlighting = {
  fill: { type: 'pattern', pattern: 'solid', bgColor: { argb: 'f8c291' } },
};
export const questionHighlighting = {
  fill: { type: 'pattern', pattern: 'solid', bgColor: { argb: 'ffffff' } },
};
export const labelHighlighting = {
  fill: { type: 'pattern', pattern: 'solid', bgColor: { argb: 'c6e0b4' } },
};
export const otherHighlighting = {
  fill: { type: 'pattern', pattern: 'solid', bgColor: { argb: '99dcff' } },
};
export const disabledHighlighting = {
  fill: { type: 'pattern', pattern: 'solid', bgColor: { argb: 'b2bec3' } },
};

export const validWorksheets = [
  'Instructions',
  'Template',
  'Release Notes',
  'Mapping',
...
export const HNQIS2_VALUE_TYPES = [
  'NUMBER',
  'INTEGER',
  'INTEGER_POSITIVE',
...
export const TRACKER_TEA_CONDITIONAL_FORMAT_VALIDATIONS = {
  ...
  teaNotFound: {
    formula:
      'OR(ISBLANK($B3), $B3 = "Not Found", ISBLANK($D3), $D3 = "Not Found", ISBLANK($E3), $E3 = "Not Found", ISBLANK($F3), $F3 = "Not Found")',
    prompt: 'The specified TEA is not available.',
  },
  disabledFutureDate: {
    formula:
      'AND($A3 = "TEA", NOT(ISBLANK($C3)), OR(ISBLANK($E3), AND($E3 <> "DATE", $E3 <> "DATETIME")))',
    dynamicFormula:
      'AND($A_ROWNUM_ = "TEA", NOT(ISBLANK($C_ROWNUM_)), OR(ISBLANK($E_ROWNUM_), AND($E_ROWNUM_ <> "DATE", $E_ROWNUM_ <> "DATETIME")))',
...
export const TRACKER_STAGE_CONDITIONAL_FORMAT_VALIDATIONS = {
  ...
  formNameNotDefined: {
    ...
    dynamicFormula:
      'AND(ISBLANK($D_ROWNUM_),NOT(ISBLANK($A_ROWNUM_)),$C_ROWNUM_="Yes")',
    prompt: 'Form Name not defined.',
  },
  nameNotDefined: {
    formula: 'AND(ISBLANK($E3),$A3 = "Data Element",$C3 = "No")',
    dynamicFormula:
      'AND(ISBLANK($E_ROWNUM_),$A_ROWNUM_ = "Data Element",$C_ROWNUM_ = "No")',
    prompt: 'Name not defined.',
  },
  shotNameNotDefined: {
    formula: 'AND(ISBLANK($F3),$A3 = "Data Element",$C3 = "No")',
    dynamicFormula:
      'AND(ISBLANK($F_ROWNUM_),$A_ROWNUM_ = "Data Element",$C_ROWNUM_ = "No")',
  ...
  valueTypeNotDefined: {
    ...
    dynamicFormula:
      'AND(ISBLANK($J_ROWNUM_),$A_ROWNUM_ = "Data Element",ISBLANK($L_ROWNUM_))',
    prompt: 'Value Type not defined.',
  },
  incompleteParentLogic: {
    formula: 'OR(AND($O3<>"", $P3=""), AND($O3="", $P3<>""))',
    dynamicFormula:
      'OR(AND($O_ROWNUM_<>"", $P_ROWNUM_=""), AND($O_ROWNUM_="", $P_ROWNUM_<>""))',
  ...
  parentNotFound: {
    formula:
      'AND($A3 = "Data Element",NOT(ISNUMBER(MATCH($O3, B:B, 0))),$O3<>"")',
    dynamicFormula:
      'AND($A_ROWNUM_ = "Data Element",NOT(ISNUMBER(MATCH($O_ROWNUM_, B:B, 0))),$O_ROWNUM_<>"")',
...
export const HNQIS2_CONDITIONAL_FORMAT_VALIDATIONS = {
  parentNameNotDefined: {
    ...
    dynamicFormula:
      'AND(ISBLANK($A_ROWNUM_),OR($B_ROWNUM_="question",$B_ROWNUM_="label"))',
  ...
  valueTypeNotDefined: {
    ...
    dynamicFormula:
      'AND($B_ROWNUM_ = "question",ISBLANK($G_ROWNUM_),ISBLANK($F_ROWNUM_))',
    prompt: 'Value Type not defined.',
  },
  valueTypeDisable: {
  ...
  feedbackOrderNotDefined: {
    formula:
      'OR(AND($B3="question", OR(NOT(ISBLANK($I3)),NOT(ISBLANK($J3))), ISBLANK($K3)), AND($B3="score", ISBLANK($K3)))',
    dynamicFormula:
      'OR(AND($B_ROWNUM_="question", OR(NOT(ISBLANK($I_ROWNUM_)),NOT(ISBLANK($J_ROWNUM_))), ISBLANK($K_ROWNUM_)), AND($B_ROWNUM_="score", ISBLANK($K_ROWNUM_)))',
    prompt: 'Feedback Order not defined.',
  },
  incompleteScoring: {
    formula: 'OR(AND($I3<>"",$J3=""), AND($I3="",$J3<>""))',
    dynamicFormula:
      'OR(AND($I_ROWNUM_<>"",$J_ROWNUM_=""), AND($I_ROWNUM_="",$J_ROWNUM_<>""))',
    prompt: 'Incomplete Numerator or Denominator.',
  },
  incompleteParentLogic: {
    formula: 'OR(AND($L3<>"", $M3=""), AND($L3="", $M3<>""))',
    dynamicFormula:
      'OR(AND($L_ROWNUM_<>"", $M_ROWNUM_=""), AND($L_ROWNUM_="", $M_ROWNUM_<>""))',
    prompt: 'Incomplete Parent Logic.',
  },
  selfParent: {
  ...
  parentNotFound: {
    formula:
      'AND(OR($B3 = "question",$B3 = "label"),NOT(ISNUMBER(MATCH($L3, A:A, 0))),$L3<>"")',
    dynamicFormula:
      'AND(OR($B_ROWNUM_ = "question",$B_ROWNUM_ = "label"),NOT(ISNUMBER(MATCH($L_ROWNUM_, A:A, 0))),$L_ROWNUM_<>"")',
...
export const getPromptsFormula = (validationsList, rowNumber) => {
  ...
  formula =
    formula +
    Object.keys(validationsList)
      .map((teaValidation) => {
        if (validationsList[teaValidation].dynamicFormula) {
          return `IF(${validationsList[teaValidation].dynamicFormula.replaceAll(
            '_ROWNUM_',
            rowNumber
          )}, "${validationsList[teaValidation].prompt} ", "")`;
        }
      })
      .join(',');
