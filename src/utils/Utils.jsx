...
export function number2Character(numb) {
  ...
  return (
    (quo !== 0 ? String.fromCharCode(64 + quo) : '') +
    String.fromCharCode(65 + numb)
  );
}
...
export function parseErrors(response) {
  ...
  return (
    errorRes.typeReports
      ...
      .flat() || (
      <p>
        {response.httpStatus}: {response.message}.
      </p>
    )
  );
}
...
export function parseErrorsJoin(response, joinVal) {
  ...
  return (
    errorRes.typeReports
      ...
      .flat()
      .join(joinVal) || response.httpStatus + ': ' + response.message + '.'
  );
}
...
export function truncateString(strVal, characters = 50, useEllipsis = true) {
  return (
    strVal.substring(0, characters) +
    (strVal.length > characters && useEllipsis ? '...' : '')
  );
}
...
export const getPCAMetadataDE = (dataElement) => {
  const jsonData = dataElement.attributeValues?.find(
...
export const setPCAMetadata = (scopeObject, newMetadata) => {
  const pcaMetadataIndex = scopeObject.attributeValues.findIndex(
    (att) => att.attribute.id == METADATA
  );
  if (pcaMetadataIndex > -1) {
    scopeObject.attributeValues[pcaMetadataIndex].value =
...
export const removeWindowsForbiddenCharacters = (string) =>
  string.replaceAll(/[*?:\\/[\]]+/g, '');
...
export { HNQIS_TYPES, HNQIS_VERSIONS, MODERN_HNQIS_INDEX };

export const isHnqisProgramType = (type) => HNQIS_VERSIONS.includes(type);

export const isModernHnqisProgramType = (type) =>
  HNQIS_VERSIONS.indexOf(type) >= MODERN_HNQIS_INDEX;

export const getHnqisType = (type) => HNQIS_TYPES[type];

export const isHnqisPCAType = (type) => Object.keys(HNQIS_TYPES).includes(type);

export const getHnqisPCAType = (type) => {
  const invertedHnqisTypes = Object.fromEntries(
    Object.entries(HNQIS_TYPES).map(([key, value]) => [value, key])
  );
