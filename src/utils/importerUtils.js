...
import {
  HNQIS2_TEMPLATE_MAP,
  HQNIS2_PROGRAM_TYPE_CELL,
  TEMPLATE_PROGRAM_TYPES,
  TRACKER_PROGRAM_TYPE_CELL,
...
export const isTracker = (importType) =>
  [TEMPLATE_PROGRAM_TYPES.tracker, TEMPLATE_PROGRAM_TYPES.event].includes(
    importType
  );
...
export const getProgramDetailsHNQIS2 = (ws, mappingDetails) => {
  ...
  const result = mappingDetails.programs.filter(
    (prog) => prog.name === program.name
  );
...
export const serverAndVersionValidation = (
  status,
  task,
  {
  ...
  if (templateVersion === releaseNotes.at(-1).version) {
    ...
    if (originServer === location.origin) {
      ...
      task.name = `The Template has been exported from a different server (${originServer.toString()}). Please Import a template downloaded from the current server (${
        location.origin
      })`;
      setNotificationError(true);
    }
...
export const workbookValidation = (
  status,
  task,
  { setNotificationError, workbook, isTracker, programSpecificType }
) => {
  const templateWS = [];
  let teasWS;
  let instructionsWS;
  ...
  workbook.eachSheet((worksheet) => {
    ...
    if (worksheet.getCell('L1').value === 'Program TEA Id') {
      ...
      worksheet.getCell('R1').value === 'Stage ID' ||
      worksheet.getCell('A1').value === 'Parent Name'
    ) {
      ...
      switch (id) {
        case 'I':
          ...
          if (isTracker) {
            isEvent =
              worksheet.getCell(TRACKER_PROGRAM_TYPE_CELL).value ===
              TEMPLATE_PROGRAM_TYPES.event;
          }
          break;
        case 'M':
          mappingWS = worksheet;
...
const worksheetValidation = (
  status,
  task,
  { setNotificationError, sheetName, headers, templateHeadersList }
) => {
...
const getWorksheetData = (
  status,
  task,
  { currentWorksheet, templateHeadersList, structureColumn, isTrackerTemplate }
) => {
  const templateData = [];
  let stageId;
  const dataRow = 3;
  ...
  if (isTrackerTemplate) {
    stageId = currentWorksheet.getCell('R2').value;
  }

  return { status, data: templateData, stageId };
};

export const handleWorksheetReading = ({
  tasksHandler,
  currentWorksheet,
  setNotificationError,
  headers,
  ...
  startingIndex,
  structureColumn,
  isTrackerTemplate = false,
}) => {
  if (
    !tasksHandler(
      {
        step: startingIndex,
        message: `${currentWorksheet.name}: Validating worksheet columns`,
        initialStatus: true,
      },
      worksheetValidation,
      {
  ...
  return tasksHandler(
    {
      step: startingIndex + 1,
      message: `Extracting data from '${currentWorksheet.name}'`,
      initialStatus: true,
    },
    getWorksheetData,
    {
...
export const mapImportedDEHNQIS2 = ({
  data,
  programPrefix,
  type,
  optionSets,
  ...
  if (type == 'score') {
    ...
    code =
      programPrefix + (data[HNQIS2_TEMPLATE_MAP.parentName]?.result || '???');
  }

  parsedDE.code = code;
  parsedDE.name = (code + data[HNQIS2_TEMPLATE_MAP.formName]).slice(
    0,
    MAX_FORM_NAME_LENGTH
  );
  parsedDE.shortName = (code + data[HNQIS2_TEMPLATE_MAP.formName])?.slice(
    0,
    MAX_SHORT_NAME_LENGTH
  );
  ...
  parsedDE.attributeValues =
    existingDe?.attributeValues?.filter(
      (att) =>
  ...
  if (type === 'question') {
    ...
    if (
      data[HNQIS2_TEMPLATE_MAP.optionSet] &&
      data[HNQIS2_TEMPLATE_MAP.optionSet] !== ''
    ) {
      const os = getObjectByProperty(
        data[HNQIS2_TEMPLATE_MAP.optionSet],
        optionSets,
        'optionSet'
      );
    ...
    if (
      data[HNQIS2_TEMPLATE_MAP.legend] &&
      data[HNQIS2_TEMPLATE_MAP.legend] !== ''
    ) {
      parsedDE.legendSet = {
        id: getObjectIdByProperty(
          data[HNQIS2_TEMPLATE_MAP.legend],
          legendSets,
          'legendSet'
        ),
    ...
    if (data[HNQIS2_TEMPLATE_MAP.scoreNum] !== '') {
      metadata.scoreNum = data[HNQIS2_TEMPLATE_MAP.scoreNum];
    }
    if (data[HNQIS2_TEMPLATE_MAP.scoreDen] !== '') {
      metadata.scoreDen = data[HNQIS2_TEMPLATE_MAP.scoreDen];
    }
  }

  if (
    data[HNQIS2_TEMPLATE_MAP.feedbackOrder] &&
    data[HNQIS2_TEMPLATE_MAP.feedbackOrder] !== ''
  ) {
    parsedDE.attributeValues.push(
      buildAttributeValue(
        FEEDBACK_ORDER,
        String(data[HNQIS2_TEMPLATE_MAP.feedbackOrder])
      )
    );
  }
  ...
  if (data[HNQIS2_TEMPLATE_MAP.parentValue] !== '') {
    metadata.parentValue = data[HNQIS2_TEMPLATE_MAP.parentValue];
  }

  if (type == 'label') {
    metadata.labelFormName = data[HNQIS2_TEMPLATE_MAP.formName];
  }
...
export const mapImportedDE = ({
  ...
  const autoNaming =
    data[TRACKER_TEMPLATE_MAP.autoNaming] === 'No' ? false : true;
  const stagePrefix = `_PS${stageNumber}`;
  const code = autoNaming
    ? programPrefix +
      stagePrefix +
      (data[TRACKER_TEMPLATE_MAP.correlative]?.result || '???')
  ...
  parsedDE.name = autoNaming
    ? (code + data[TRACKER_TEMPLATE_MAP.formName]).slice(
        0,
        MAX_FORM_NAME_LENGTH
      )
  ...
  parsedDE.domainType = 'TRACKER';
  parsedDE.valueType = getKeyByValue(
    DHIS2_VALUE_TYPES_MAP,
    data[TRACKER_TEMPLATE_MAP.valueType]
  );
  parsedDE.aggregationType = getKeyByValue(
    DHIS2_AGG_OPERATORS_MAP,
    data[TRACKER_TEMPLATE_MAP.aggOperator] || 'None'
  );
  parsedDE.parentName = data[TRACKER_TEMPLATE_MAP.correlative]?.result
    ? stagePrefix + data[TRACKER_TEMPLATE_MAP.correlative]?.result
    : undefined;
  parsedDE.attributeValues =
    existingDe?.attributeValues?.filter(
      (att) => ![METADATA].includes(att.attribute.id)
    ) || [];

  if (
    data[TRACKER_TEMPLATE_MAP.optionSet] &&
    data[TRACKER_TEMPLATE_MAP.optionSet] !== ''
  ) {
    const os = getObjectByProperty(
      data[TRACKER_TEMPLATE_MAP.optionSet],
  ...
  if (
    data[TRACKER_TEMPLATE_MAP.legend] &&
    data[TRACKER_TEMPLATE_MAP.legend] !== ''
  ) {
    parsedDE.legendSet = {
      id: getObjectIdByProperty(
  ...
  if (
    data[TRACKER_TEMPLATE_MAP.parentQuestion] &&
    data[TRACKER_TEMPLATE_MAP.parentQuestion] !== ''
  ) {
    metadata.parentQuestion =
      stagePrefix + data[TRACKER_TEMPLATE_MAP.parentQuestion];
    parsedDE.parentQuestion =
      stagePrefix + data[TRACKER_TEMPLATE_MAP.parentQuestion]; // TO BE REPLACED WITH PARENT DATA ELEMENT'S UID
  }
...
export const countChanges = ({
  ...
  const removedObjects = currentData
    .map((sec) => {
      // Section removed -> Increase counter
      if (!sections.find((i_sec) => i_sec[impSectionId] == sec[sectionId])) {
        sectionsSummary.removed++;
      }
      return sec[countObject].filter(
        (obj) =>
          !sections.find((i_sec) =>
            i_sec[countObject].find((i_obj) => i_obj[impObjId] == obj[objId])
          )
      );
    })
    .flat();
...
export const getBasicForm = (type) => {
  ...
  if (type === 'TEA') {
    result.trackedEntityAttributes = [];
  }
  if (type === 'DE') {
    result.dataElements = [];
  }
