...
export const validateMetadataStructure = ( prgStgSections ) => {
    ...
    //         errMsgList.push(`The section '${secItem.ref_source.displayName}' must have at least one 'Standard'.`);
    //     }
    //     else
    //     {
    ...
    {
        if ( secItem.standards.length === 0 ) {
            secItem.ref_source.errors = generateErrorMessage( "Standard", "NOT FOUND", `The section '${secItem.ref_source.displayName}' must have at least one 'Standard'.`);
        }
        else
        {
            secItem.standards.forEach( ( stdItem ) => {
...
const generateErrorMessage = (title, code, text) => {
    return {
        title: title,
