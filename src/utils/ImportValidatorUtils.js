...
export const HNQIS2_VALIDATION_SETTINGS = {
  sections: {
    ...
    checkHasName: {
      enabled: true,
      title: 'Section Form Name is not defined',
      errorMsg: {
        code: 'EXW113',
  ...
  programDetails: {
    ...
    checkHasFormName: {
      enabled: true,
      title: 'Element Form Name is not defined',
      errorMsg: {
        code: 'EXW100',
    ...
    checkFormNameLength: {
      enabled: true,
      title: 'Form name length not valid',
      errorMsg: {
        code: 'EXW112',
    ...
    structureMatchesValue: {
      enabled: false,
      title: 'Label should be LONG_TEXT',
      errorMsg: {
        code: 'EXW103',
    ...
    hasBothNumeratorDenominator: {
      enabled: true,
      title: 'Numerator or Denominator missing',
      errorMsg: {
        code: 'EXW106',
    ...
    validAggregationType: {
      enabled: true,
      title: 'Aggregation Type Not Valid',
      errorMsg: {
        code: 'EW104',
    ...
    matchesScore: {
      enabled: true,
      title: 'Score container not found',
      errorMsg: {
        code: 'EXW110',
    ...
    checkHasValueType: {
      enabled: true,
      title: 'Question Value Type is not defined',
      errorMsg: {
        code: 'EXW114',
        text: 'A Value Type was not defined for the specified question.',
      },
    },
    checkNotSelfParent: {
      enabled: true,
      title: 'Invalid Parent Question',
      errorMsg: {
        code: 'EXW115',
        text: 'A Question cannot be a Parent to itself.',
      },
    },
    checkParentExists: {
      enabled: true,
      ...
      errorMsg: {
        code: 'EXW116',
        text: 'The specified Parent does not exist in the Assessment.',
      },
    },
  },
  scores: {
  ...
  feedbackOrder: {
    ...
    checkGaps: {
      enabled: true,
      title: 'Feedback Order gap found',
      errorMsg: {
        code: 'EW106',
    ...
    checkDuplicated: {
      enabled: true,
      title: 'Duplicated Feedback Order found',
      errorMsg: {
        code: 'EW107',
...
export const TRACKER_VALIDATION_SETTINGS = {
  sections: {
    ...
    checkHasName: {
      enabled: true,
      title: 'Section Name is not defined',
      errorMsg: {
        code: 'EXWT100',
        text: 'A Name was not defined for the specified Section.',
      },
    },
  },
  teas: {
    ...
    checkHasName: {
      enabled: true,
      title: 'Element Name is not defined',
      errorMsg: {
        code: 'EXWT101',
        text: 'A Name was not defined for the specified element.',
      },
    },
    checkDuplicated: {
      enabled: true,
      title: 'Duplicated TEA',
      errorMsg: {
        code: 'EXWT102',
        text: 'The specified Tracked Entity Attribute is duplicated.',
      },
    },
  },
  dataElements: {
    ...
    checkHasCorrelative: {
      enabled: true,
      title: 'Invalid Data Element Correlative',
      errorMsg: {
        code: 'EXWT102',
        text: 'The Correlative for the specified Data Element is not valid.',
      },
    },
    checkHasFormName: {
      enabled: true,
      title: 'Data Element Form Name is not defined',
      errorMsg: {
        code: 'EXWT103',
        text: 'A Form Name was not defined for the specified Data Element.',
      },
    },
    checkFormNameLength: {
      enabled: true,
      title: 'Data Element Form Name length out of range',
      errorMsg: {
        code: 'EXWT104',
        text: `Given Form Name length is out of the accepted range (Between ${MIN_DATA_ELEMENT_NAME_LENGTH} and ${MAX_TRACKER_DATA_ELEMENT_NAME_LENGTH} characters).`,
      },
    },
    checkHasName: {
      enabled: true,
      title: 'Data Element Name is not defined',
      errorMsg: {
        code: 'EXWT105',
        text: 'A Name was not defined for the specified Data Element.',
      },
    },
    checkNameLength: {
      enabled: true,
      title: 'Data Element Name length out of range',
      errorMsg: {
        code: 'EXWT106',
        text: `Given Name length is out of the accepted range (Between ${MIN_DATA_ELEMENT_NAME_LENGTH} and ${MAX_TRACKER_DATA_ELEMENT_NAME_LENGTH} characters).`,
      },
    },
    checkHasShortName: {
      enabled: true,
      title: 'Data Element Short Name is not defined',
      errorMsg: {
        code: 'EXWT107',
        text: 'A Short Name was not defined for the specified Data Element.',
      },
    },
    checkShortNameLength: {
      enabled: true,
      title: 'Data Element Short Name length out of range',
      errorMsg: {
        code: 'EXWT108',
        text: `Given Short Name length is out of the accepted range (Less than ${MAX_SHORT_NAME_LENGTH} characters).`,
      },
    },
    checkCodeLength: {
      enabled: true,
      title: 'Data Element Code length out of range',
      errorMsg: {
        code: 'EXWT109',
        text: `Given Code length is out of the accepted range (Less than ${MAX_SHORT_NAME_LENGTH} characters).`,
      },
    },
    checkHasValueType: {
      enabled: true,
      title: 'Data Element Value Type is not defined',
      errorMsg: {
        code: 'EXWT110',
        text: 'A Value Type was not defined for the specified Data Element.',
      },
    },
    checkHasParentLogic: {
      enabled: true,
      title: 'Incomplete Data Element Parent Logic',
      errorMsg: {
        code: 'EXWT111',
        text: 'One of the components of the Parent Logic is missing (Parent Data Element or Answer Value).',
      },
    },
    checkNotSelfParent: {
      enabled: true,
      title: 'Invalid Parent Data Element',
      errorMsg: {
        code: 'EXWT112',
        text: 'A Data Element cannot be a Parent to itself.',
      },
    },
    checkParentExists: {
      enabled: true,
      ...
      errorMsg: {
        code: 'EXWT113',
        text: 'The specified Parent does not exist in the Stage of the Data Element.',
      },
    },
  },
};
    ...
    .map((section) => {
      return section.dataElements
        .map((de) => ({
          id: de.id,
          code: de.code,
          feedbackOrder: de.attributeValues.find(
...
export function verifyProgramDetail(importResults, setValidationMessage) {
  if (importResults) {
    if (
      importResults.program.id !== null &&
      importResults.program.name !== null
    ) {
      const programDetail = importResults.mapping.programs.filter(
        (prog) => prog.name === importResults.program.name
      );
      if (
        programDetail.length > 0 &&
        programDetail[0]?.id === importResults.program.id
      ) {
        return true;
      }
    }
    setValidationMessage(
      'Program Name and ID are not valid. Please check the settings in the Instructions Tab.'
    );
    return false;
  }
  return true;
...
const checkDuplicatedFeedbacks = (hnqisMode, scores) => {
  if (hnqisMode) {
    return scores.filter((score) =>
      scores.find(
        (match) =>
          match.feedbackOrder === score.feedbackOrder &&
          match.code !== score.code
      )
    );
  }
  return [];
...
const buildFeedbackErrorObject = (message, instance, elementError) => {
  ...
  errorObject.title =
    ...
    ': ' +
    (!errorObject.instance.expectedValues
      ? errorObject.instance.elements.join(', ')
      : errorObject.instance.expectedValues.join(' or ')) +
    '.';
...
export function validateFeedbacks(hnqisMode, sections) {
  ...
  const feedbackOrderValidationSettings =
    HNQIS2_VALIDATION_SETTINGS.feedbackOrder;
  if (feedbackOrderValidationSettings.enabled) {
    const feedbackData = getFeedbackData(sections);
    const duplicatedFeedbacks = groupByKey(
      checkDuplicatedFeedbacks(hnqisMode, feedbackData),
      'feedbackOrder'
    );
    if (feedbackOrderValidationSettings.checkDuplicated.enabled) {
      duplicatedFeedbacks.forEach((df) => {
        feedbacksErrors.push(
          buildFeedbackErrorObject(
            feedbackOrderValidationSettings.checkDuplicated.errorMsg,
            df,
            feedbackOrderValidationSettings.duplicatedFO
          )
        );
      });
    }
    ...
    if (feedbackOrderValidationSettings.checkGaps.enabled) {
      ...
      while (index < feedbackData.length - 1) {
        ...
        if (result) {
          ...
          feedbacksErrors.push(
            buildFeedbackErrorObject(
              feedbackOrderValidationSettings.checkGaps.errorMsg,
              instance,
              feedbackOrderValidationSettings.gapFO
            )
          );
        }
...
export const checkFormNameLength = ({ metadata, dataElement }) => {
  ...
  if (metadata.elem !== '') {
    return (
      formName.replace(' [C]', '').length <= MAX_DATA_ELEMENT_NAME_LENGTH &&
      formName.replace(' [C]', '').length >= MIN_DATA_ELEMENT_NAME_LENGTH
    );
  }
  return true;
};

export const structureMatchesValue = ({
  metadata,
  dataElement,
  element,
  valueType,
  ...
  if (metadata.elemType === element) {
    return dataElement.valueType === valueType;
  }
  return true;
};
...
export const hasFeedbackOrder = ({ metadata, dataElement }) => {
  if (
    hasAttributeValue(metadata, 'scoreNum') &&
    hasAttributeValue(metadata, 'scoreDen')
  ) {
...
export const hasVarName = ({ metadata }) => {
  return (
    hasAttributeValue(metadata, 'varName') &&
    isValidParentName(metadata, 'varName')
  );
};
...
export const hasCorrelative = ({ metadata }) => {
  return (
    hasAttributeValue(metadata, 'varName') &&
    isValidCorrelative(metadata, 'varName')
  );
};
...
export const parentIsNotSelf = ({ metadata }) => {
  if (
    hasAttributeValue(metadata, 'varName') &&
    hasAttributeValue(metadata, 'parentQuestion')
  ) {
...
export const parentExists = ({ metadata, dataElementsList }) => {
  ...
  return dataElementsList.find((dataElement) => {
    ...
    return (
      deMetadata.varName === metadata.parentQuestion ||
      dataElement.id === metadata.parentQuestion
    );
  });
};

export const questionMatchesScore = ({ importedScores, dataElement }) => {
  let feedbackOrder = dataElement.attributeValues.find(
  ...
  const compositeScores = importedScores.dataElements.map(
    (score) =>
      score.attributeValues.find((att) => att.attribute.id == FEEDBACK_ORDER)
        ?.value
  );
...
export const getFeedbackOrder = (dataElement) => {
  const feedbackOrder = dataElement.attributeValues.filter(
    (attributeValue) => attributeValue.attribute.id === FEEDBACK_ORDER
  );
  return feedbackOrder.length > 0 ? feedbackOrder[0].value : '';
};

export const validAggregationType = ({
  metadata,
  dataElement,
  element,
...
export const checkDuplicatedTEA = ({ tea, teaList }) => {
  return (
    tea !== 'Not Found' &&
    teaList.filter((element) => element === tea).length > 1
  );
};
...
export const validateQuestions = (
  ...
  if (validations.enabled) {
    ...
    validate(
      validations.checkHasFormName,
      checkHasFormName,
      { metadata, dataElement },
      errors
    );
    validate(
      validations.checkFormNameLength,
      checkFormNameLength,
      { metadata, dataElement },
      errors
    );
    validate(
      validations.structureMatchesValue,
      structureMatchesValue,
      { metadata, dataElement, element: 'label', valueType: 'LONG_TEXT' },
      errors
    );
    validate(
      validations.hasFeedbackOrder,
      hasFeedbackOrder,
      { metadata, dataElement },
      errors
    );
    ...
    validate(
      validations.checkHasValueType,
      checkHasProperty,
      { object: dataElement, property: 'valueType' },
      errors
    );
    validate(
      validations.hasBothNumeratorDenominator,
      hasBothNumeratorDenominator,
      { metadata, dataElement },
      errors
    );
    validate(
      validations.validAggregationType,
      validAggregationType,
      { metadata, dataElement, element: 'label', aggregationOperation: 'NONE' },
      errors
    );
    validate(
      validations.validAggregationQuestion,
      validAggregationQuestion,
      { metadata, dataElement },
      errors
    );
    validate(
      validations.isNumeratorNumeric,
      isNumeric,
      { metadata, property: 'scoreNum' },
      errors
    );
    validate(
      validations.isDenominatorNumeric,
      isNumeric,
      { metadata, property: 'scoreDen' },
      errors
    );
    validate(
      validations.hasParentQuestionNAnswerValue,
      hasBothParentLogicComponents,
      { metadata },
      errors
    );
    validate(
      validations.matchesScore,
      questionMatchesScore,
      { importedScores, dataElement },
      errors
    );
    validate(
      validations.checkNotSelfParent,
      parentIsNotSelf,
      { metadata },
      errors
    );
    validate(
      validations.checkParentExists,
      parentExists,
...
export const validateSectionsHNQIS2 = (section, errorDetails) => {
  ...
  if (validations.enabled) {
    ...
    validate(
      validations.checkHasName,
      checkSectionHasFormName,
      { section },
      errors
    );
...
export const validateScores = (dataElement, errorDetails) => {
  ...
  if (validations.enabled) {
    ...
    validate(
      validations.structureMatchesValue,
      structureMatchesValue,
      { metadata, dataElement, element: 'score', valueType: 'NUMBER' },
      errors
    );
    validate(
      validations.hasScoreFeedbackOrder,
      hasScoreFeedbackOrder,
      { metadata, dataElement },
      errors
    );
...
export const validateTEA = (tea, teaList, errorDetails) => {
  ...
  validate(
    validations.checkHasName,
    checkHasProperty,
    { object: tea.trackedEntityAttribute, property: 'name' },
    errors
  );
  validate(
    validations.checkDuplicated,
    checkDuplicatedTEA,
    { tea: tea.trackedEntityAttribute.id, teaList: [...teaList] },
    errors,
    false
  );
...
export const validateDataElement = (
  dataElement,
  errorDetails,
  dataElementsList
) => {
  ...
  validate(
    ...
    checkPropertyLength,
    {
      object: dataElement,
      property: 'formName',
      min: MIN_DATA_ELEMENT_NAME_LENGTH,
      max: MAX_TRACKER_DATA_ELEMENT_NAME_LENGTH,
    },
    errors
  );
  validate(
    validations.checkHasCorrelative,
    hasCorrelative,
    { metadata },
    errors
  );
  if (metadata.autoNaming === 'No') {
    validate(
      validations.checkHasName,
      checkHasProperty,
      { object: dataElement, property: 'name' },
      errors
    );
    validate(
      validations.checkNameLength,
      checkPropertyLength,
      {
        object: dataElement,
        property: 'name',
    ...
    validate(
      validations.checkHasShortName,
      checkHasProperty,
      { object: dataElement, property: 'shortName' },
      errors
    );
    validate(
      validations.checkShortNameLength,
      checkPropertyLength,
      {
        object: dataElement,
        property: 'shortName',
        min: 0,
        max: MAX_SHORT_NAME_LENGTH,
      },
      errors
    );
    validate(
      validations.checkCodeLength,
      checkPropertyLength,
      {
        object: dataElement,
        property: 'code',
    ...
    validate(
      validations.checkHasFormName,
      checkHasProperty,
      { object: dataElement, property: 'formName' },
      errors
    );
  }
  validate(
  ...
  validate(
    validations.checkHasParentLogic,
    hasBothParentLogicComponents,
    { metadata },
    errors
  );
...
export const getNewObjectsCount = (importResults) =>
  extractAttributeValues(importResults, 'new').reduce(
...
const mapDataElements = (dataElements, stage, { stageNumber, sectionIdx }) => {
  return dataElements.map((de, deIdx) => {
    ...
    metadata.varName =
      metadata.varName ||
      `_PS${padValue(stageNumber, '00')}_S${padValue(
        sectionIdx + 1,
        '00'
      )}E${padValue(deIdx + 1, '000')}`;
    de.parentName = metadata.varName;
    if (metadata.parentQuestion) {
      metadata.parentQuestion = getVarNameFromParentUid(
        metadata.parentQuestion,
        stage,
        false
      );
      ...
      input.programSections.map((section) => ({
        ...
        trackedEntityAttributes: mapAttributes(
          section.trackedEntityAttributes.map((teaId) =>
            input.programTrackedEntityAttributes.find(
              (ptea) => ptea.trackedEntityAttribute.id === teaId.id
            )
          )
        ),
      }))
