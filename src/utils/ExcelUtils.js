...
export function printArray2Column(
  sheet,
  array,
  header,
  startPosition,
  ...
  for (let i = 0; i < array.length; i++) {
    pos =
      coordinates[0] + (parseInt(coordinates[1]) + parseInt(i) + parseInt(1));
    sheet.getCell(pos).value = array[i];
  }

  applyBorderToRange(
    sheet,
    character2Number(coordinates[0]),
    coordinates[1],
    character2Number(coordinates[0]),
    parseInt(coordinates[1]) + parseInt(array.length)
  );
}
export function printObjectArray(sheet, datas, position, headerBgColor) {
  ...
  applyBorderToRange(
    sheet,
    character2Number(excelColumns.at(0)),
    headerRow,
    character2Number(excelColumns.at(-1)),
    parseInt(row) - 1
  );
}
...
export function fillBackgroundToRange(sheet, range, backgroundColor) {
  ...
  for (let i = parseInt(start[1]); i <= parseInt(end[1]); i++) {
    for (
      let j = character2Number(start[0]);
      j <= character2Number(end[0]);
      j++
    ) {
      const colChar = number2Character(j);
      const cellAxis = colChar + i.toString();
...
export const formatDate = (date) => {
  const formattedDate = date
    .toLocaleString('en-US', SHORT_DATE_FORMAT_OPTIONS)
    .split(', ');
...
export const hideColumns = (ws, columnsList) => {
  columnsList.forEach((column) => (ws.getColumn(column).hidden = true));
};
...
export const getTrackerMappingList = (ws) => {
  return {
    trackedEntityAttributes: getMappedValues(ws, 3, 'D', {
      id: 'D',
      tea: 'C',
      aggType: 'F',
      valueType: 'G',
    }),
    optionSets: getMappedValues(ws, 3, 'K', {
      id: 'K',
      optionSet: 'J',
      valueType: 'L',
    }),
...
export const getHNQIS2MappingList = (ws) => {
  return {
    optionSets: getMappedValues(ws, 3, 'I', {
      id: 'I',
      optionSet: 'H',
      valueType: 'J',
    }),
...
export const getVarNameFromParentUid = (
  parentUid,
  programStage,
  removeStagePrefix = true
) => {
  const parentDe = programStage.programStageSections
    .map((pss) => pss.dataElements)
    .flat()
    .find((de) => de.id == parentUid);
  const deMetadata = JSON.parse(
    parentDe?.attributeValues?.find((av) => av.attribute.id === METADATA)
      ?.value || '{}'
  );
